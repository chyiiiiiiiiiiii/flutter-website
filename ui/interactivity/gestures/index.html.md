...
若要直接從元件層監聽指標事件，請使用 [`Listener`][] 元件。
...
`onPanStart`
: 指標已接觸螢幕，並可能開始水平或垂直移動。若同時設定 `onHorizontalDragStart` 或 `onVerticalDragStart`，此回呼（callback）會發生錯誤。
...
許多這類元件已經能回應點擊或手勢。
例如，[`IconButton`][] 與 [`TextButton`][]
會回應按壓（點擊），而 [`ListView`][]
則可回應滑動以觸發捲動。
如果您沒有使用這些元件，但希望在點擊時有「墨水波紋」效果，可以使用 [`InkWell`][]。
