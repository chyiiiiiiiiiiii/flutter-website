...
<DashImage figure img-class="site-mobile-screenshot border" image="ui/layout/layout-demo-app.png" caption="完成的應用程式畫面。" width="50%" />
...
<DashImage figure img-class="site-mobile-screenshot border" image="ui/layout/layout-sketch-intro.svg" caption="版面配置中的主要元素：圖片、列、列，以及文字區塊" width="50%" />
...
<DashImage figure image="ui/layout/layout-sketch-title-block.svg" caption="含有文字區塊與圖示的標題區塊" />
...
<DashImage figure image="ui/layout/layout-sketch-button-block.svg" caption="含有三個標籤按鈕的按鈕區塊" width="50%" />
...
<DashImage figure image="ui/layout/layout-sketch-title-block-unlabeled.svg" caption="標題區塊的草圖與原型 UI" />
...
在 [`SingleChildScrollView`][] 元件內，將 `Text` 元件替換為 `Column` 元件。
...
* 如果遇到問題，請將你的程式碼與 [`lib/main.dart`][] 進行比較。
:::

[automatic reformatting support]: /tools/formatting
[hot reload]: /tools/hot-reload
[`lib/main.dart`]: https://github.com/flutter/website/blob/main/examples/layout/lakes/step2/lib/main.dart
...
<DashImage figure image="ui/layout/layout-sketch-button-block-unlabeled.svg" caption="按鈕區塊的草圖與原型 UI" />
...
1. 傳遞該按鈕專屬的顏色、`Icon` 和文字。
1. 使用 `MainAxisAlignment.spaceEvenly` 值，沿著主軸（main axis）對齊這些欄位（columns）。
...
<DashImage figure image="ui/layout/layout-sketch-add-text-block.svg" caption="文字區塊的草圖與原型 UI" />
...
將 [`softWrap`][] 設定為 `true` 時，文字行會在單字邊界自動換行前，先填滿整個欄寬。
...
1. 下載 [`lake.jpg`][] 圖片並將其加入新建立的 `images` 目錄。

   :::note
   你無法使用 `wget` 來儲存這個二進位檔案。
   你可以從 [Unsplash][] 依據 Unsplash 授權條款下載這張[圖片][ch-photo]。
   這張小尺寸圖片大小為 94.4 kB。
   :::
...
在 `children` 清單中，新增一個 `ImageSection` 元件作為第一個子元件。
將 `image` 屬性設為你在
[設定應用程式以使用提供的圖片](#configure-your-app-to-use-supplied-images) 中新增的圖片路徑。
...
<DashImage figure img-class="site-mobile-screenshot border" image="ui/layout/layout-demo-app.png" caption="完成的應用程式" width="50%" />

## 資源

你可以從以下位置取得本教學中使用的資源：

**Dart 程式碼：** [`main.dart`][]<br>
**圖片：** [ch-photo][]<br>
