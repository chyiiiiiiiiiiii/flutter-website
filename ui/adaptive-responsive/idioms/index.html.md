...
## 考慮各平台上的預期行為 {:#consider-expected-behavior-on-each-platform}
...
### 尋找平台倡議者 {:#find-a-platform-advocate}
...
## 常見的慣用語與規範 {:#common-idioms-and-norms-to-consider}

快速瀏覽幾個你可能需要考慮的特定規範與慣用語，以及你可以如何在 Flutter 中實現它們。

### 捲軸條（Scrollbar）的外觀與行為 {:#scrollbar-appearance-and-behavior}

桌面與行動裝置的使用者都期望有捲軸條 (scrollbar)，但他們對其行為的預期在不同平台上有所不同。行動裝置使用者預期捲軸條較小，且僅在捲動時出現；而桌面使用者通常預期捲軸條始終可見、較大，且可以點擊或拖曳。

Flutter 內建的 `Scrollbar` 元件 (Widget) 已經根據當前平台支援自適應的顏色與尺寸。你可能想要調整的一點是，在桌面平台上切換 `alwaysShown`：
...
### 多重選取（Multi-select） {:#multi-select}
...
幸運的是，這可以很容易地透過 [`SelectableText`][] 元件 (Widget) 來支援：
...
這在 Flutter 中並未直接支援，但你可以使用 [`bits_dojo`][] 套件來停用原生標題列，並以你自訂的標題列取代。
...
### 快顯選單與工具提示（Context menus and tooltips） {:#context-menus-and-tooltips}
...
若要在 Flutter 中顯示基本的工具提示，可使用內建的 [`Tooltip`][] 元件 (Widget)：
...
若要顯示更進階的工具提示 (tooltip)、彈出面板 (popup panel)，或建立自訂的快顯選單，你可以選擇使用現有的套件，或是自行利用 `Stack` 或 `Overlay` 來實作。
...
### 水平按鈕順序 {:#horizontal-button-order}

在 Windows 上，當呈現一排按鈕時，確認按鈕會放在這一排的開頭（左側）。而在其他所有平台上，則相反，確認按鈕會放在這一排的結尾（右側）。

在 Flutter 中，可以透過 `Row` 上的 `TextDirection` 屬性輕鬆處理這個需求：
...
如果你同時需要自訂標題列和選單列，
可以考慮在 Flutter 中自行實作，
類似於自訂快顯選單 (context menu) 的方式。

### 拖放（Drag and drop） {:#drag-and-drop}
...
這是因為在觸控操作下，捲動與拖曳都共用同一根手指作為輸入。

滑鼠使用者則有更多輸入選項。他們可以使用滾輪或捲軸來捲動，
通常就不需要專門的拖曳把手。
如果你觀察 macOS Finder 或 Windows Explorer，
會發現它們就是這樣運作：只要選取一個項目並開始拖曳即可。

在 Flutter 中，你可以用多種方式實作拖放功能。
本文不會深入討論具體實作細節，但高層級的選項包括以下幾項：

* 直接使用 [`Draggable`][] 和 [`DragTarget`][] API，
  以打造自訂的外觀與體驗。
...
[pre-made list packages]: https://pub.dev/packages?q=reorderable+list
