...
[file an issue]: https://github.com/flutter/website/issues/new
[pull request]: https://github.com/flutter/website/pulls
...
在 Windows 上執行 `flutter doctor` 時，你可能會看到類似以下的錯誤：
...
此錯誤通常發生於 `SystemRoot` 環境變數遺失，或 PowerShell 執行原則阻止腳本正常執行時。
...
2.  **檢查環境變數**：
    確認 `SystemRoot` 環境變數已設定（通常設定為 `C:\Windows`）。你可以在 PowerShell 終端機中執行 `echo $env:SystemRoot` 來檢查其值。

3.  **檢查執行原則**：
    如果問題仍然存在，你可能需要調整執行原則。
    在以系統管理員身分開啟的 PowerShell 視窗中執行以下指令：
