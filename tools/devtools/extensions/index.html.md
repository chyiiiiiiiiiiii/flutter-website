...
> 學習如何使用與建立 DevTools 擴充功能。



## 什麼是 DevTools 擴充功能？

[DevTools extensions][]（DevTools 擴充功能）是由第三方套件所提供的開發者工具，並且與 DevTools 工具套件緊密整合。擴充功能會作為 pub 套件的一部分進行發佈，當使用者在除錯應用程式時，這些擴充功能會動態載入到 DevTools 中。
...
:::note
如果該擴充功能需要連線至正在執行的應用程式，
你將不會看到啟用提示或啟用設定，
直到 DevTools 已連線到正在執行的應用程式為止。
:::
...
如需深入了解如何建立 DevTools 擴充功能的詳細指南，請參閱
