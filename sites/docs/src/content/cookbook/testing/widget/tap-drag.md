---
title: 點擊、拖曳與輸入文字
description: 如何測試元件（Widget）的使用者互動。
---

<?code-excerpt path-base="cookbook/testing/widget/tap_drag/"?>

許多元件 (Widget) 不僅用來顯示資訊，也會回應使用者的互動。這包括可點擊的按鈕，以及用於輸入文字的 [`TextField`][]。

為了測試這些互動行為，你需要在測試環境中模擬這些操作。為此，請使用 [`WidgetTester`][] 函式庫。
...
在許多情況下，使用者互動會更新應用程式的狀態 (state)。在測試環境中，Flutter 並不會在狀態變更時自動重建元件。為了確保在模擬使用者互動後元件樹會被重建，請呼叫 `WidgetTester` 所提供的 [`pump()`][] 或 [`pumpAndSettle()`][] 方法。
本教學將採用以下步驟：
...
你可以透過以下步驟完成這個任務：
  ...
  1. 在測試環境中建置元件 (Widget)。
  2. 使用 `WidgetTester` 的 [`enterText()`][] 方法。
...
最後，請確認對待辦事項項目執行滑動刪除（swipe-to-dismiss）操作時，該項目會從清單中移除。這包含三個步驟：
  ...
  2. 使用 [`pumpAndSettle()`][] 方法，持續重建元件樹，直到刪除動畫完成。
  3. 確認該項目已不再顯示於螢幕上。
