...
![SnackBar 範例](/assets/images/docs/cookbook/snackbar.webp){:.site-mobile-screenshot}
...
當你開發遵循 Material Design 指南的應用程式時，應確保應用程式具有一致的視覺結構。在這個範例中，將 `SnackBar` 顯示在螢幕底部，且不會與其他重要元件 (Widget) 重疊，例如 `FloatingActionButton`。
...
:::note
想了解更多內容，請觀看這段關於 `ScaffoldMessenger` 元件的兩分鐘 Widget of the Week 短片：
...
例如，若使用者不小心刪除了訊息，他們可以透過 `SnackBar` 中的 `action` 來恢復該訊息。

以下是一個為 `SnackBar` 元件提供額外 `action` 的範例：
...
<noscript>
  <img src="/assets/images/docs/cookbook/snackbar.webp"
    alt="SnackBar 範例" class="site-mobile-screenshot" />
</noscript>
