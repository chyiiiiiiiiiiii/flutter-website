...
本範例使用 [`sqflite`](https://pub.dev/packages/sqflite) 搭配 [`sqflite_common_ffi`](https://pub.dev/packages/sqflite_common_ffi) 套件，兩者結合可同時支援行動裝置與桌面端。Web 支援則由實驗性套件 [`sqflite_common_ffi_web`](https://pub.dev/packages/sqflite_common_ffi_web) 提供，但本範例未涵蓋。
