...
作為暫時性的解決方案，
你可以考慮使用 [Firebase Hosting][] 或 [GitHub Pages][]。
...
* 在 [developer account][] 中找到 team ID。
...
1. 將 `paths` 陣列設為 `["*"]`。
   ...
   如有需要，請將 `paths` 陣列的值調整為更適合你應用程式的設定。
...
:::note
如果你有多個 scheme/flavor，可以在 `appIDs` 欄位中
加入多個 `appID`。
:::
...
你可以在 GitHub 儲存庫中找到 [deeplink_cookbook][]
範例的原始碼。

[apple-app-site-assoc]: https://developer.apple.com/documentation/xcode/supporting-associated-domains
