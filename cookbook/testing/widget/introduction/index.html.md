...
[`flutter_test`](https://api.flutter.dev/flutter/flutter_test/flutter_test-library.html) 套件（隨 Flutter SDK 一同提供）所提供的額外工具。

`flutter_test` 套件為元件（Widget）測試提供了以下工具：

  * [`WidgetTester`](https://api.flutter.dev/flutter/flutter_test/WidgetTester-class.html) 可在測試環境中建立並互動操作元件（Widget）。
  * [`testWidgets()`](https://api.flutter.dev/flutter/flutter_test/testWidgets.html) 函式會自動為每個測試案例建立新的 `WidgetTester`，
    並取代一般的 `test()` 函式使用。
  * [`Finder`](https://api.flutter.dev/flutter/flutter_test/Finder-class.html) 類別可用於在測試環境中搜尋元件（Widget）。
  * 元件專用的 [`Matcher`](https://api.flutter.dev/flutter/package-matcher_matcher/Matcher-class.html) 常數有助於驗證
...
使用`flutter_test`套件所提供的[`testWidgets()`](https://api.flutter.dev/flutter/flutter_test/testWidgets.html)函式來定義一個測試。
...
接下來，請利用 `WidgetTester` 所提供的 [`pumpWidget()`](https://api.flutter.dev/flutter/flutter_test/WidgetTester/pumpWidget.html) 方法，在測試環境中建立 `MyWidget`。
...
為此，請使用 `flutter_test` 套件提供的頂層 [`find()`](https://api.flutter.dev/flutter/flutter_test/find-constant.html) 方法來建立 `Finders`。由於你知道要找的是 `Text` 元件（Widgets），因此可以使用 [`find.text()`](https://api.flutter.dev/flutter/flutter_test/CommonFinders/text.html) 方法。
...
為此，請使用 [`findsOneWidget`](https://api.flutter.dev/flutter/flutter_test/findsOneWidget-constant.html) `Matcher`。
...
[`find.text()`]: https://api.flutter.dev/flutter/flutter_test/CommonFinders/text.html
...
[`Matcher`]: https://api.flutter.dev/flutter/package-matcher_matcher/Matcher-class.html
這些技巧將幫助你撰寫更可靠且可維護的 Flutter 應用程式。

[`pumpWidget()`]: https://api.flutter.dev/flutter/flutter_test/WidgetTester/pumpWidget.html
