...
單元測試（Unit test）非常適合用來驗證單一函式、方法或類別的行為。[`test`][] 套件提供了撰寫單元測試的核心框架，而 [`flutter_test`][] 套件則提供了更多用於測試元件 (Widget) 的工具。
...
如需有關 test 套件的更多資訊，請參閱 [test package documentation][]。
...
如果你想要執行一系列相關的測試，可以使用 `flutter_test` 套件的 [`group`][] 函式來將測試分類。將測試歸入同一群組後，你可以透過一個指令，對該群組中的所有測試呼叫 `flutter test`。
...
[`group`]: https://api.flutter.dev/flutter/flutter_test/group.html
