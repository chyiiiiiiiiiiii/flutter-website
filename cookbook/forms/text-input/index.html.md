...
你可以透過提供 [`InputDecoration`][] 作為 `TextField` 的 [`decoration`][] 屬性，
...
[`TextFormField`][] 包裝了一個 `TextField`，並將其與外層的 [`Form`][] 整合。
這樣可以提供額外的功能，例如驗證，以及與其他
[`FormField`][] 元件 (Widgets) 的整合。
...
[建立具有驗證功能的表單][Building a form with validation] 教學範例。


[Building a form with validation]: /cookbook/forms/validation/
[`decoration`]: https://api.flutter.dev/flutter/material/TextField/decoration.html
