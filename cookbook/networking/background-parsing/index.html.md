...
預設情況下，Dart 應用程式會在單一執行緒上執行所有工作。
在多數情境下，這種模型能簡化程式設計，且速度足夠快，
不會造成應用程式效能不佳或動畫卡頓，
這種情況通常被稱為「jank」。
...
為了避免 jank，你需要將這類耗時運算
移到背景執行，並使用獨立的 [Isolate][Isolate]。
本教學將採用以下步驟：
...
從 [JSONPlaceholder REST API][JSONPlaceholder REST API] 取得一份包含 5000 筆照片物件的大型 JSON 文件。
...
接下來，根據
[Fetch data from the internet][Fetch data from the internet] 教學的指引，
將 `http.Response` 轉換為 Dart 物件的清單。
這樣可以讓資料更容易操作。

### 建立 `Photo` 類別

首先，建立一個包含照片資料的 `Photo` 類別。
請加入一個 `fromJson()` 工廠方法（factory method），
...
如果你在較慢的裝置上執行 `fetchPhotos()` 函式，你可能會注意到應用程式在解析與轉換 JSON 時會短暫凍結。這就是所謂的 jank（卡頓），你會希望消除這種現象。

你可以透過使用 Flutter 提供的 [`compute()`][`compute()`] 函式，將解析與轉換的工作移至背景 isolate 來消除卡頓。`compute()` 函式會在背景 isolate 執行耗時的函式，並回傳結果。在這個案例中，請將 `parsePhotos()` 函式於背景執行。
...
Isolate 之間是透過傳遞訊息來進行溝通。這些訊息可以是原始型別的值，例如 `null`、`num`、`bool`、`double` 或 `String`，也可以是像本範例中的 `List<Photo>` 這樣的簡單物件。
...
[`http.get()`]: https://pub.dev/documentation/http/latest/http/get.html
[Isolate]: https://api.flutter.dev/flutter/dart-isolate/Isolate-class.html
