...
本教學將說明如何使用 [`http.get()`][] 方法，從 [JSONPlaceholder][] 取得一個範例相簿。
...
* [`Future`][] 是 Dart 的核心類別之一，用於處理非同步 (async) 操作。Future 物件代表一個未來某個時間點可能會取得的值或錯誤。
...
使用 [pattern matching][] 來轉換 JSON 只是其中一種方式。
如需更多資訊，請參閱完整文章
...
如果你希望能在 [`InheritedWidget`][] 變動時重新載入 API，請將呼叫放在
`didChangeDependencies()` 方法中。
更多細節請參考 [`State`][]。
...
要在螢幕上顯示資料，請使用
[`FutureBuilder`][] 元件（Widget）。
`FutureBuilder` 元件 (Widget) 是 Flutter 內建的，
讓你可以輕鬆處理非同步資料來源。
...
將 `fetchAlbum()` 的結果儲存在狀態 (state) 變數中，可以確保
