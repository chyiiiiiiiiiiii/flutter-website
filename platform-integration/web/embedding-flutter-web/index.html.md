# 將 Flutter 加入任何網頁應用程式

> 了解將 Flutter 視圖嵌入網頁內容的不同方式。
...
* 當視圖被新增或移除時，Flutter 應用程式會收到通知，因此可以相應地調整其元件 (Widget)。
...
[`didChangeMetrics` method]: https://api.flutter.dev/flutter/widgets/WidgetsBindingObserver/didChangeMetrics.html
[Multi View Playground repo]: https://github.com/goderbauer/mvp
[type `FlutterView`]: https://api.flutter.dev/flutter/dart-ui/FlutterView-class.html
[`View` widget]: https://api.flutter.dev/flutter/widgets/View-class.html
[`ViewCollection` widget]: https://api.flutter.dev/flutter/widgets/ViewCollection-class.html
...
[`runApp` function]: https://api.flutter.dev/flutter/widgets/runApp.html
[`runWidget` function]: https://api.flutter.dev/flutter/widgets/runWidget.html
...
[`View.of` constructor]: https://api.flutter.dev/flutter/widgets/View/of.html
...
在 Dart 中，`initialData` 以 `JSAny` 物件的形式提供，可透過 `dart:ui_web` 函式庫中的頂層 `views` 屬性存取。資料可透過目前視圖的 `viewId` 存取，如下所示：
...
[JS Interoperability]: https://dart.dev/interop/js-interop

### 視圖約束（View constraints）

預設情況下，嵌入式 Flutter Web 視圖會將其 `hostElement` 的大小視為不可變屬性，並將其版面配置嚴格限制在可用空間內。

在 Web 上，元素的內在大小通常會影響頁面的版面配置（例如 `img` 或 `p` 標籤，可以使內容重新排版）。

當你將視圖新增至 Flutter Web 時，可以透過約束條件來告知 Flutter 該視圖需要如何進行版面配置：
...
[`ViewConstraints` class]: https://api.flutter.dev/flutter/dart-ui/ViewConstraints-class.html
