...
:::note
本教學示範了 Flutter 對 [Material 3][Material 3] 以及
[google_fonts][google_fonts] 套件的支援。自 Flutter 3.16 版本起，
...
每個主題會定義顏色、字型樣式及其他適用於各種 Material 元件 (Material components) 的參數。
...
Flutter 的 Material 元件 (Material components) 會根據你的主題設定
...
此屬性需要一個 [`ThemeData`][`ThemeData`] 實例。
...
如果你希望應用程式中的某個元件（component）忽略整體主題，
可以建立一個`ThemeData`實例。
...
要延伸主題，請使用 [`copyWith()`][`copyWith()`] 方法。
