...
但設計師通常希望能支援自訂字型。
你可能有設計師專門設計的自訂字型，
或者你從 [Google Fonts][Google Fonts] 下載了字型。
...
想進一步了解主題化（theming），
請參考 [Using Themes to share colors and font styles][Using Themes to share colors and font styles] 教學。
...
:::important
如果一個 [`TextStyle`][`TextStyle`] 物件指定了字重（weight）或字型樣式（style），但沒有對應的字型檔案，Flutter 引擎會使用通用的字型檔案，並嘗試推算出所要求的字重和樣式的輪廓。
