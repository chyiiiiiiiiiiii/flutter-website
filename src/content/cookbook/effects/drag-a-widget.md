...
在這個案例中，當使用者長按 `MenuListItem` 元件時，`LongPressDraggable` 元件會顯示一個 `DraggingListItem`。  
...
在這個案例中，`data` 的型別是 `Item`，它保存了使用者所點擊的菜單項目的相關資訊。
...
`DragTarget` 會顯示你現有的元件（Widget），同時與 `LongPressDraggable` 協作，以辨識使用者何時將可拖曳項目拖曳到 `DragTarget` 上方。
...
當你將 `DragTarget` 元件設定為接受你想要的資料後，就可以透過拖放的方式，將資料從 UI 的一個部分傳送到另一個部分。
...
當使用者將選單項目拖放到`CustomerCart`元件（Widget）上時，`_itemDroppedOnCustomerCart`方法會在`onAcceptWithDetails()`中被呼叫。透過將拖放的項目加入`customer`物件，並呼叫`setState()`以觸發版面配置（layout）更新，UI 會隨即刷新，顯示新的顧客總價與項目數量。
