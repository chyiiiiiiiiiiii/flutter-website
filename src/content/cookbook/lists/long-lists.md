...
標準的 [`ListView`][`ListView`] 建構函式（constructor）非常適合用於小型清單。若要處理包含大量項目的清單，建議使用 [`ListView.builder`][`ListView.builder`] 建構函式。
...
在本範例中，使用 [`List.generate`][`List.generate`] 建構函式產生一個包含 10,000 筆字串的清單。
...
若要指定每個項目的範圍（extent），你可以使用 [`prototypeItem`][`prototypeItem`]、[`itemExtent`][`itemExtent`] 或 [`itemExtentBuilder`][`itemExtentBuilder`]。

明確指定範圍會比讓子項目自行決定範圍來得更有效率，因為滾動機制可以利用預先得知的子項目範圍來減少運算量，例如當滾動位置大幅變動時。

如果你的清單（list）中所有項目大小固定，請使用 [`prototypeItem`][`prototypeItem`] 或 [`itemExtent`][`itemExtent`]。

如果你的清單中項目大小不一，請使用 [`itemExtentBuilder`][`itemExtentBuilder`]。
