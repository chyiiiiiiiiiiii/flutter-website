---
title: 使用占位符淡入圖片
description: 如何讓圖片淡入顯示。
---

<?code-excerpt path-base="cookbook/images/fading_in_images"?>

當你使用預設的 `Image` 元件（Widget）來顯示圖片時，
你可能會注意到圖片在載入時會直接出現在螢幕上，
這對使用者來說可能會感覺有些突兀。

那麼，能否一開始先顯示一個占位符（placeholder），
等圖片載入完成後再淡入顯示呢？你可以使用
[`FadeInImage`][`FadeInImage`] 元件（Widget）來達成這個效果。

`FadeInImage` 可以搭配各種類型的圖片使用：記憶體內圖片、本地資源（assets），
或是來自網路的圖片。
...
你也可以考慮使用本地資源作為預設佔位圖（placeholder）。
首先，將資源加入專案的 `pubspec.yaml` 檔案
（更多細節請參閱 [Adding assets and images][Adding assets and images]）：
