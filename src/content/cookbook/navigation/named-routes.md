...
你已學會如何透過建立新的 Route 並將其推送到 [`Navigator`][`Navigator`] 來導覽至新螢幕。
...
接下來，透過在 [`MaterialApp`][`MaterialApp`] 建構函式中提供額外的屬性，來定義路由：`initialRoute`
以及 `routes` 本身。
...
當元件（Widgets）與命名路由（routes）都已設定好後，可以透過 [`Navigator.pushNamed()`][`Navigator.pushNamed()`] 方法來觸發導航。
