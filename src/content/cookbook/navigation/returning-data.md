...
在某些情境下，你可能會希望從新螢幕返回資料。
例如，假設你推送了一個新螢幕，並向使用者呈現兩個選項。
當使用者點擊其中一個選項時，你希望能通知第一個螢幕
使用者的選擇，以便根據該資訊進行處理。

你可以透過 [`Navigator.pop()`][`Navigator.pop()`]
方法，依照以下步驟來實現：
...
現在，請更新兩個按鈕的 `onPressed()` 回呼函式（callback）。
若要將資料回傳到第一個螢幕，
請使用 [`Navigator.pop()`][`Navigator.pop()`] 方法，
此方法可接受一個名為 `result` 的可選第二個參數。
任何結果都會回傳給 SelectionButton 中的 `Future`。
...
## 5. 在主畫面顯示選擇結果的 snackbar

現在你已經能夠啟動選擇螢幕並等待結果返回，
你會希望對回傳的資訊做些處理。

在這個例子中，可以透過在 `SelectionButton` 中使用 `_navigateAndDisplaySelection()` 方法，
來顯示一個 snackbar 並展示結果：
...
<noscript>
  <img src="/assets/images/docs/cookbook/returning-data.webp" alt="返回資料示範" class="site-mobile-screenshot" />
</noscript>
