...
大多數應用程式都包含多個螢幕，用於顯示不同類型的資訊。例如，一個應用程式可能有一個顯示產品的螢幕。當使用者點擊某個產品的圖片時，會跳轉到一個新螢幕，顯示該產品的詳細資訊。

:::note Terminology
在 Flutter 中，_螢幕_ 和 _頁面_ 被稱為 _路由 (Route)_。
本教學後續將以「路由」來稱呼。
:::
...
本教學使用 [`Navigator`][`Navigator`] 來導航至新路由。
...
首先，建立兩個路由來進行操作。由於這是一個基礎範例，每個路由僅包含一個按鈕。在第一個路由上點擊按鈕會導航到第二個路由；在第二個路由上點擊按鈕則會返回第一個路由。
...
若要切換到新的路由，請使用 [`Navigator.push()`][`Navigator.push()`] 方法。`push()` 方法會將 `Route` 加入由 `Navigator` 管理的路由堆疊（stack）中。那麼，`Route` 是從哪裡來的呢？
你可以自行建立，也可以使用特定平台的路由，例如 [`MaterialPageRoute`][`MaterialPageRoute`] 或 [`CupertinoPageRoute`][`CupertinoPageRoute`]。
特定平台的路由很有用，因為它會以該平台專屬的動畫（Animation）切換到新路由。

在 `FirstRoute` 元件（Widget）的 `build()` 方法中，更新 `onPressed()` 回呼函式（callback）：
...
`pop()` 方法會將目前的 `Route` 從由 `Navigator` 管理的 Route 堆疊中移除。
...
本主題中的範例展示了一種使用 [`push`] 和 [`pop`] 方法，透過 [`Navigator`] 類別來導覽至新螢幕並返回前一個場景的方法。不過，`Navigator` 還有其他幾個靜態方法可供使用。以下是其中幾個：

*   [`pushAndRemoveUntil`]：將一個導覽路由加入堆疊，然後移除堆疊中最新的路由，直到符合特定條件為止。
*   [`pushReplacement`]：以新路由取代堆疊頂端的當前路由。
*   [`replace`]：將堆疊中的某個路由以另一個路由取代。
*   [`replaceRouteBelow`]：取代堆疊中特定路由下方的路由。
*   [`popUntil`]：從導覽路由堆疊中移除最近加入的路由，直到符合特定條件為止。
*   [`removeRoute`]：從堆疊中移除特定路由。
*   [`removeRouteBelow`]：移除堆疊中特定路由下方的路由。
*   [`restorablePush`]：還原從堆疊中移除的路由。
