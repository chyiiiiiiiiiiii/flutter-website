...
在應用程式開發中，播放影片是一項常見的任務，Flutter 應用程式也不例外。為了播放影片，Flutter 團隊提供了 [`video_player`][`video_player`] 套件。你可以使用 `video_player` 套件來播放儲存在檔案系統、資源（asset）或網路上的影片。

:::warning
目前，
`video_player` 套件尚不支援 Linux 與 Windows 平台。
如需進一步了解，請參考 [`video_player`][`video_player`] 套件。
:::

在 iOS 平台上，`video_player` 套件會使用 [`AVPlayer`][`AVPlayer`] 來處理播放功能；而在 Android 平台上，則會使用 [`ExoPlayer`][`ExoPlayer`]。
...
現在，來顯示影片。`video_player` 套件提供了 [`VideoPlayer`][`VideoPlayer`] 元件（Widget），用於顯示由 `VideoPlayerController` 初始化的影片。
預設情況下，`VideoPlayer` 元件會佔據盡可能多的空間。
但這通常不適合影片，因為影片通常需要以特定的長寬比顯示，例如 16x9 或 4x3。

因此，請將 `VideoPlayer` 元件包裹在 [`AspectRatio`][`AspectRatio`] 元件中，以確保影片能以正確的比例顯示。
...
請呼叫 `VideoPlayerController` 提供的 [`play()`][`play()`] 方法。
若要暫停播放，則呼叫 [`pause()`][`pause()`] 方法。
