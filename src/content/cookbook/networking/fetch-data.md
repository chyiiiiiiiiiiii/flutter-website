...
本教學將說明如何使用 [`http.get()`][`http.get()`] 方法，從 [JSONPlaceholder][JSONPlaceholder] 取得範例相簿資料。
...
* [`Future`][`Future`] 是 Dart 的核心類別，用於處理非同步操作。Future 物件代表一個未來某個時間點可能可用的值或錯誤。
* `http.Response` 類別包含從成功的 HTTP 呼叫中接收到的資料。

## 3. 將回應轉換為自訂 Dart 物件

雖然發送網路請求很簡單，但直接處理原始的 `Future<http.Response>` 並不方便。
為了讓開發更容易，請將 `http.Response` 轉換成 Dart 物件。

### 建立 `Album` 類別

首先，建立一個 `Album` 類別，用來儲存網路請求取得的資料。這個類別包含一個 factory 建構子，可從 JSON 建立 `Album`。

使用 [pattern matching][pattern matching] 轉換 JSON 只是其中一種方式。
如需更多資訊，請參閱完整文章：[JSON and serialization][JSON and serialization]。
...
現在，請依照以下步驟，將 `fetchAlbum()`
函式更新為回傳 `Future<Album>`：

  1. 使用 `dart:convert` 套件，將回應主體轉換為 JSON `Map`。
  2. 如果伺服器回傳的狀態碼為 200 的 OK 回應，則使用 `fromJson()` 工廠方法，將 JSON `Map` 轉換為 `Album`。
...
太棒了！
你現在已經有一個可以從網路上擷取專輯資料的函式了。

## 4. 擷取資料

在 [`initState()`][`initState()`] 或 [`didChangeDependencies()`][`didChangeDependencies()`] 方法中呼叫 `fetchAlbum()` 方法。

`initState()` 方法只會被呼叫一次，之後就不會再執行。
如果你希望能夠在 [`InheritedWidget`][`InheritedWidget`] 發生變化時重新載入 API，請將呼叫放在 `didChangeDependencies()` 方法中。
更多細節請參考 [`State`][`State`]。
...
請注意，`snapshot.hasData` 只有在 snapshot
包含非 null 的資料值時才會回傳 `true`。

由於 `fetchAlbum` 只能回傳非 null 的值，
所以即使遇到「404 Not Found」伺服器回應，
該函式也應該拋出例外。
拋出例外會將 `snapshot.hasError` 設為 `true`，
這可以用來顯示錯誤訊息。

否則，則會顯示載入中的旋轉圖示（spinner）。
...
雖然這樣做很方便，
但不建議將 API 呼叫放在 `build()` 方法中。

Flutter 會在每次需要
變更畫面時呼叫 `build()` 方法，
而這種情況其實發生得非常頻繁。
如果將 `fetchAlbum()` 方法放在 `build()` 內，
每次重建（rebuild）時都會重複呼叫，
導致應用程式效能下降。

將 `fetchAlbum()` 的結果儲存在狀態變數中，可以確保
`Future` 只會執行一次，並在後續重建時快取該結果。
...
請參考以下教學：
  ...
  * [單元測試簡介][Introduction to unit testing]
  * [使用 Mockito 模擬相依物件][Mock dependencies using Mockito]
