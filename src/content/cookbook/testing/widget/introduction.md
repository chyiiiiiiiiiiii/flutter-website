...
[`flutter_test`][`flutter_test`] 套件（隨 Flutter SDK 一同提供）所提供的額外工具。
...
為此，請使用 `flutter_test` 套件提供的頂層 [`find()`][`find()`] 方法來建立 `Finders`。由於你知道要找的是 `Text` 元件（Widgets），因此可以使用 [`find.text()`][`find.text()`] 方法。

如需關於 `Finder` 類別的更多資訊，請參閱 [Finding widgets in a widget test][Finding widgets in a widget test] 教學。
