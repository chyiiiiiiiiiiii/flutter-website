...
文字欄位 (text field) 讓使用者可以在應用程式中輸入文字。
它們常用於建立表單、傳送訊息、打造搜尋體驗等多種情境。
在本教學中，將介紹如何建立並設計文字欄位。

Flutter 提供了兩種文字欄位：
[`TextField`][`TextField`] 和 [`TextFormField`][`TextFormField`]。
...
來新增標籤 (label)、圖示 (icon)、內嵌提示文字 (inline hint text) 以及錯誤訊息 (error text)。
