...
width="100%" alt="完成的 Rolodex 聯絡人管理應用程式截圖，顯示依字母順序排列的聯絡人清單。">

完成本教學後，你將學會如何建立自適應版面、實作完整的主題化、建構導覽模式，以及運用進階的滾動技術。
...
* 同時支援淺色與深色主題。
* 使用 Cupertino 元件 (Widgets) 打造 iOS 風格的 UI。

本教學假設你已完成前面的 Flutter 教學，並且熟悉基本的元件組合、狀態管理，以及 Flutter 專案結構。

## 建立新的 Flutter 專案

要開發 Flutter 應用程式，首先需要一個 Flutter 專案。你可以使用 [Flutter CLI 工具][Flutter CLI tool]（隨 Flutter SDK 一併安裝）來建立新的應用程式。

請開啟終端機或命令提示字元，執行以下指令來建立新的 Flutter 專案：
...
此專案會使用 [`cupertino_icons` 套件][`cupertino_icons` package]，這是一個官方的 Flutter 套件。請透過執行下列指令，將其加入為相依套件：
...
與前兩個教學不同，這個應用程式使用`CupertinoApp`，而不是`MaterialApp`。Cupertino 設計系統提供 iOS 風格的元件（Widgets）與樣式，非常適合打造在 Apple 裝置上具有原生體驗的應用程式。
...
應用程式會在一個新的 Chrome 實例中建置並啟動，並在螢幕中央顯示
「Hello Rolodex!」。
...
這份範例資料包含了有中間名和字尾，以及沒有中間名和字尾的聯絡人。這讓你在建立 UI 時，可以操作多樣化的資料。
