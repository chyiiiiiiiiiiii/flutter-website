...
歡迎來到 Flutter 教學課程！本教學將帶你從零開始學習如何建立可在行動裝置、桌面與網頁上運行的應用程式。

你將從最基礎開始：建立一個空白的 Flutter 應用程式。到課程結束時，你將完成數個小型應用程式，這些應用程式將展示 Flutter 開發的關鍵功能（以及更多！）
...
Flutter 是一個開源的 UI 工具包（UI toolkit），可協助你從單一程式碼庫建構可原生編譯、表現力豐富的應用程式，涵蓋行動裝置、網頁與桌面平台。Flutter 採用宣告式、響應式設計，並具備 hot reload 快速開發循環，以及豐富且可自訂的元件（Widgets）集，讓你能打造具表現力的介面。

Flutter 會自行繪製每一個像素，而非包裝原生元件，這讓開發者能完全掌控 UI，並確保跨平台的視覺一致性。

## 如何使用本教學

為了順利跟上本教學，你應該已熟悉 Dart 程式語言。本教學假設你已具備 [Learn Dart 教學][Learn Dart tutorial] 的所有相關知識。（或者，如果你熟悉其他通用的物件導向語言，如 Java 或 Kotlin，也大致沒問題。）

## 設定環境

閱讀本教學時，建議你能跟著範例一起實作。你可以選擇[在你的電腦上安裝 Flutter][installing Flutter on your machine]，或使用支援 Flutter 的網頁 IDE —— [Firebase Studio][Firebase Studio]。
...
1. Flutter UI 入門
   ...
   3. [在螢幕上配置元件（Widgets）][Layout widgets on a screen]
...
3. Flutter UI 進階（102）
   ...
   2. [`LayoutBuilder` 與自適應版面（adaptive layouts）][`LayoutBuilder` and adaptive layouts]
   3. [滾動與 sliver][Scrolling and slivers]
