...
:::note
如果您尚未安裝 Flutter，
請先參閱並依照 [開始使用 Flutter][Get started with Flutter] 指南進行操作。
...
如果您尚未安裝，
請安裝並設定最新穩定版本的 [Android Studio][Android Studio]。
 ...
 1. <h3>安裝 Android SDK 與工具</h3>
   ...
   1. <h3>同意 Android 授權條款</h3>
      ...
      1. 閱讀並接受所有必要的授權條款。
         ...
         如果你尚未先前接受過每一份 SDK 授權條款，
         在開發 Android 應用程式前，你需要審閱並同意這些條款。
...
你可以在實體 Android 裝置上進行 Flutter 應用程式的除錯，或是在 Android 模擬器上執行它們。
...
若要設定開發環境以在 Android 模擬器上執行 Flutter 應用程式，請依照以下步驟操作：
 ...
 1. <h3>建立新的模擬器</h3>
    ...
    1. 如果有提供選項，
       請根據你的開發電腦是 x64 還是 Arm64 裝置，選擇 **x86 Images** 或 **ARM Images**。
    ...
    1. 確認你的虛擬裝置設定。
       ...
       想了解更多虛擬裝置資訊，
       請參考 [Create and manage virtual devices][Create and manage virtual devices]。
...
若要設定開發環境以在實體 Android 裝置上執行 Flutter 應用程式，請依照以下步驟操作：
 ...
 1. <h3>啟用無線除錯</h3>
    ...
    若要使用無線除錯，
    請依照 [Connect to your device using Wi-Fi][Connect to your device using Wi-Fi] 的說明，
    在你的裝置上啟用 **無線除錯**。
    {: .windows-only}

 1. <h3>安裝平台相依前置需求</h3>

    如果你是在 Windows 上開發，請先依照
    [Install OEM USB drivers][Install OEM USB drivers] 的說明，
    為你的裝置安裝必要的 USB 驅動程式。
 ...
 1. <h3>確認裝置連線</h3>
    ...
    若要確認 Flutter 是否辨識到你已連接的 Android 裝置，
...
## 驗證您的設定 {: #validate-setup}
 ...
 1. <h3>檢查工具鏈問題</h3>
    ...
    如果你在 **Android toolchain** 或 **Android Studio** 區塊下看到任何錯誤或待完成的事項，

請完成所有提及的任務，然後再次執行 `flutter doctor` 來驗證變更。

1. <h3>檢查 Android 裝置</h3>

為了確保你正確設定了模擬器和/或實體 Android 裝置，
...
你可以繼續學習 Flutter 並在 Android 上進行測試，
或是開始加強與 Android 的整合。
  ...
  <div class="card filled-card list-card">
    ...
    <div class="card-content">
      <ul>
        ...
        <li>
          <a class="text-button" href="/platform-integration/android/c-interop">綁定原生 Android 程式碼</a>
        </li>
        <li>
