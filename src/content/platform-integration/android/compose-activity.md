...
因此，如你在程式碼範例中所見，
原生 `MethodChannel` 回呼的工作就是啟動第二個活動 (activity)。
...
1. 請在檔案最上方加入以下 buildscript 區塊：
 ...
 1. 在檔案底部新增第二個 `Activity`，這個元件你已在先前對 `AndroidManifest.xml` 的修改中參考過：
...
以下步驟說明如何從 Flutter 應用程式啟動原生 Android activity，有時這是一種簡單的方法，可用來連接特定的 Android 功能。
