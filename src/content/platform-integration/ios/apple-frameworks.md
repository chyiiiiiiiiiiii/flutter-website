...
當你來自 iOS 開發背景時，可能需要尋找能夠提供與 Apple 系統函式庫相同能力的 Flutter 套件（plugin）。這可能包括存取裝置硬體或與特定框架（如 `HealthKit`）互動。

若想了解 SwiftUI 框架與 Flutter 的比較，請參閱 [Flutter for SwiftUI developers][Flutter for SwiftUI developers]。

## 介紹 Flutter 套件（plugin）

Dart 將包含平台特定程式碼的函式庫稱為 _plugin_，即「plugin package」的簡稱。
...
雖然本頁有連結至部分熱門 plugin，你還可以在 [pub.dev][pub.dev] 上找到數千個 plugin 及其範例。
下列表格並不代表推薦任何特定 plugin。
如果找不到符合需求的套件，你可以自行建立，或直接在專案中使用平台通道（platform channels）。
欲了解詳情，請參閱 [Writing platform-specific code][Writing platform-specific code]。

## 將 plugin 加入你的專案

若要在原生專案中使用 Apple 框架，請將其匯入至你的 Swift 或 Objective-C 檔案。

若要加入 Flutter plugin，請在專案根目錄執行 `flutter pub add package_name`。
這會將相依性加入你的 [`pubspec.yaml`][`pubspec.yaml`] 檔案。
加入相依性後，請在 Dart 檔案中加入該套件的 `import` 陳述式。

你可能還需要調整應用程式設定或初始化邏輯。
如有需要，[pub.dev][pub.dev] 上該套件的「Readme」頁面應會提供詳細說明。
...
| 發送網路請求                                   | `URLSession`                                                                  | [`http`][`http`]                   |
| 儲存鍵值資料                                   | `@AppStorage` property wrapper 與 `NSUserDefaults`                             | [`shared_preferences`][`shared_preferences`]     |
| 儲存至資料庫                                   | `CoreData` 或 SQLite                                                          | [`sqflite`][`sqflite`]                |
...
| 使用擴增實境                                   | `ARKit`                                                                       | [`ar_flutter_plugin`][`ar_flutter_plugin`]      |
| 存取天氣資料                                   | `WeatherKit`                                                                  | [`weather`][`weather`][^6]            |
| 存取與管理聯絡人                               | `Contacts`                                                                    | [`contacts_service`][`contacts_service`]       |
| 在主畫面提供快速動作                           | `UIApplicationShortcutItem`                                                       | [`quick_actions`][`quick_actions`]          |
| 在 Spotlight 搜尋中索引項目                    | `CoreSpotlight`                                                               | [`flutter_core_spotlight`][`flutter_core_spotlight`] |
| 設定、更新並與 Widgets 溝通                    | `WidgetKit`                                                                   | [`home_widget`][`home_widget`]            |
| 透過 Siri/捷徑自動化應用程式動作               | `AppIntents`                                                                  | [`intelligence`][`intelligence`]            |
