...
:::warning
即將到來的 iOS 變更導致 Flutter 在 iOS 26（目前為 beta 版）實體裝置上的除錯模式暫時無法運作。
如果你的實體裝置已經升級到 iOS 26，建議切換到**模擬器（Simulator）**分頁並依照指示操作。
詳情請參考 [Flutter on latest iOS][Flutter on latest iOS]。
:::
...
請為每一台你想要測試的 iOS 裝置進行設定。
 ...
 1. <h3>建立開發者簽署憑證</h3>
    ...
    你也必須加入 [Apple Developer program][Apple Developer program]
