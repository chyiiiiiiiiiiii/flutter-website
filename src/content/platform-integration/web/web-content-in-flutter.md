...
若想進一步了解如何與 DOM API 互動，請參考 [`HTMLVideoElement` 類別] 於 [`package:web`][`package:web`]。

若想進一步了解投放至 `web.HTMLVideoElement` 的影片 `Object`，請參考 Dart 的 [JS Interoperability][JS Interoperability] 文件。
...
2. 在應用程式的元件樹中，利用 `HtmlElementView('viewType')` 放置帶有指定 `viewType` 的元件
