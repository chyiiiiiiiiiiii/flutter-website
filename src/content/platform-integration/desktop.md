...
聯邦設計很實用，因為不同專業領域的團隊
可以為不同平台開發套件實作。
你可以為 pub.dev 上任何經認可的聯邦套件
新增新的平台實作，
只要你有與原始套件作者協調這項工作。

如需更多資訊，包括有關認可（endorsed）套件的說明，請參考以下資源：

* [開發套件與插件][Developing packages and plugins]，特別是
  [聯邦套件（Federated plugins）][Federated plugins] 章節。
...
：這是一個教學（codelab），將帶你一步步
建立一個將 GitHub GraphQL API
