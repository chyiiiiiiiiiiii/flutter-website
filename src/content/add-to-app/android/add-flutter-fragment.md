...
本指南說明如何將 Flutter `Fragment` 加入現有的 Android 應用程式。在 Android 中，[`Fragment`][`Fragment`] 代表一個大型 UI 的模組化部分。`Fragment` 可用於顯示滑動抽屜、分頁內容、`ViewPager` 中的一個頁面，或僅僅代表單一`Activity` 應用程式中的一般螢幕。Flutter 提供了 [`FlutterFragment`][`FlutterFragment`]，讓開發者可以在任何可使用一般 `Fragment` 的地方呈現 Flutter 體驗。
 ...
 * 應使用新的 [`FlutterEngine`][`FlutterEngine`] 還是快取的 `FlutterEngine`
...
前述程式碼已足以渲染一個 Flutter UI，該 UI 會從呼叫你的 `main()` Dart 進入點（entrypoint）開始，初始 Flutter 路由為 `/`，並建立一個新的 `FlutterEngine`。然而，這段程式碼尚不足以實現所有預期的 Flutter 行為。Flutter 依賴於各種作業系統（OS）訊號，這些訊號必須從你的主機 `Activity` 傳遞到 `FlutterFragment`。下列範例展示了這些呼叫：
...
`FlutterFragment` 會在內部知曉 [`FlutterEngineCache`][`FlutterEngineCache`]，
並根據提供給 `withCachedEngine()` 的 ID 取得預先加載（pre-warmed）的 `FlutterEngine`。

如前所示，透過提供預先加載的 `FlutterEngine`，
您的應用程式能夠盡可能快速地渲染出
第一個 Flutter 畫面。

#### 使用快取引擎（cached engine）指定初始路由
...
為了提升用戶在這段短暫等待期間的體驗，Flutter 支援顯示啟動畫面（也稱為「啟動畫面」或「launch screen」），直到 Flutter
渲染出第一個畫面為止。關於如何顯示啟動畫面的詳細說明，請參閱 [splash screen guide][splash screen guide]。
...
:::note
`FlutterFragment` 的 initial route 屬性在使用預先加載（pre-warmed）的 `FlutterEngine` 時不會產生效果，因為預先加載的 `FlutterEngine` 已經選擇了一個 initial route。當預先加載 `FlutterEngine` 時，可以明確指定 initial route。
:::

## 從指定的 entrypoint 執行 Flutter

與變更 initial route 類似，不同的 `FlutterFragment` 可能希望執行不同的 Dart entrypoint。在典型的 Flutter 應用程式中，通常只有一個 Dart entrypoint：`main()`，但你也可以定義其他 entrypoint。

`FlutterFragment` 支援指定要為特定 Flutter 體驗執行的 Dart entrypoint。
若要指定 entrypoint，請如以下方式建立 `FlutterFragment`：
...
`FlutterFragment` 設定會執行名為 `mySpecialEntrypoint()` 的 Dart entrypoint。
請注意，`()` 括號不包含在 `dartEntrypoint` `String` 名稱中。
...
在預先加載 `FlutterEngine` 時，可以明確指定要執行的 Dart entrypoint。
:::
...
如果您的應用程式有上述任一需求，則必須使用 `TextureView` 取代 `SurfaceView`。
...
預設情況下，`FlutterFragment`會以不透明的背景進行繪製，並使用`SurfaceView`。（請參閱「控制`FlutterFragment`的渲染模式」。）對於 Flutter 未繪製的像素，該背景會顯示為黑色。為了效能考量，以不透明背景進行繪製是建議的渲染模式。在 Android 上以透明方式渲染 Flutter 會對效能產生負面影響。然而，許多設計需求會要求 Flutter 畫面中的透明像素能夠顯示底層的 Android UI。為此，Flutter 支援在`FlutterFragment`中使用半透明效果。
...
關於如何控制`FlutterFragment`的渲染模式，請參閱「控制`RenderMode`的渲染模式」。
:::
...
`FlutterFragment` 提供了一個概念，協助區分何時應讓 `FlutterFragment` 能夠控制其所屬的 `Activity`，以及何時 `FlutterFragment` 只應影響自身行為。為了避免 `FlutterFragment` 將其 `Activity` 暴露給 Flutter 外掛，並防止 Flutter 控制 `Activity` 的系統 UI，請在 `FlutterFragment` 的 `Builder` 中使用 `shouldAttachEngineToActivity()` 方法，如下所示：
