...
* 如果我的動畫因為耗時的 Dart 非同步（async）函式呼叫而導致 UI 執行緒被阻塞，該如何解決？
  * 使用 [`compute()`][`compute()`] 方法建立另一個 isolate，如 [Parse JSON in the background][Parse JSON in the background] cookbook 範例所示。
...
* 如何查看 Flutter 引擎的大小組成？
  * 請造訪 [binary size dashboard][binary size dashboard]，並將網址中的 git hash 替換為 [Flutter 的 GitHub commits][Flutter's GitHub commits] 中的最新提交 hash。
...
* 如何將正在執行的應用程式截圖並匯出為 SKP 檔案？
  ...
  * 注意已知的截圖檢視問題：
    * [Issue 21237][Issue 21237]：無法在實體裝置上記錄圖片。
