...
多虧了 Flutter 的 Skia 引擎以及其能夠快速建立與銷毀元件（Widgets）的能力，
Flutter 應用程式預設就具有良好的效能，
因此您只需避免常見的陷阱，即可達到卓越的效能表現。

## 一般建議

如果您發現動畫（Animation）出現卡頓（不流暢），請**務必**在以 _profile_ 模式建置的應用程式下進行效能分析。
...
* 每一幀重建的 UI 遠超過預期。要追蹤元件（Widgets）重建情況，請參閱 [顯示效能資料][Show performance data]。
...
您是否在行動裝置應用程式上，僅在動畫（Animation）首次執行時發現明顯的卡頓？為避免這種情況，請確保您正在使用 Flutter 預設的圖形渲染器 [Impeller][Impeller]。
