---
title: Impeller 繪圖引擎
description: 什麼是 Impeller？如何啟用 Impeller？
---

:::note
自 3.27 版本起，Impeller 已成為 iOS 與 Android API 29+ 的預設
繪圖引擎。
若需查看 Impeller 目前支援平台的_詳細_資訊，
請參考 [Can I use Impeller?][Can I use Impeller?] 頁面。
:::
...
Impeller 為 Flutter 提供了一個全新的繪圖執行時。
Impeller 會在引擎建置階段預先編譯一組[更小、更簡單的著色器 (shaders)][smaller, simpler set of shaders]，
因此在執行時不需再進行編譯。

[smaller, simpler set of shaders]: {{site.repo.flutter}}/issues/77412

若想了解 Impeller 的影片介紹，請參考下方
來自 Google I/O 2023 的演講。
...
* **可預測的效能**：
  Impeller 會在建置階段離線編譯所有著色器與反射，
  並預先建立所有管線狀態物件。
  引擎會自行控制快取並明確地進行快取管理。
* **可監控 (Instrumentable)**：
  Impeller 會標記並標籤所有圖形資源，
  例如紋理（textures）與緩衝區（buffers）。
  它能將動畫（Animation）擷取並儲存到磁碟，
  而不會影響每一幀的繪製效能。
* **可移植 (Portable)**：
  Flutter 並未將 Impeller 綁定於特定的用戶端繪圖 API。
  你可以只撰寫一次著色器，並視需要轉換為後端專屬格式。
* **善用現代圖形 API**：
  Impeller 會使用（但不依賴）Metal 及 Vulkan 等現代 API 所提供的功能。
* **善用並行處理**：
  Impeller 如有需要，能將單一幀的工作分配到多個執行緒上。

## 支援情況

哪些平台可以使用 Impeller？如需_詳細_資訊，請參考
[Can I use Impeller?][Can I use Impeller?] 頁面。
...
Impeller **在 Android API 29+ 上預設啟用並可用**。
在執行較舊版本 Android 或不支援 Vulkan 的裝置上，
Impeller 會自動回退至傳統的 OpenGL 繪圖引擎。
這種回退行為無需你額外操作。

* 若在除錯時想_停用_ Impeller，
  請在 `flutter run` 指令中加入 `--no-enable-impeller` 參數。

  ```console
  flutter run --no-enable-impeller
  ```

* 若要在部署應用程式時**停用** Impeller，請在專案的 `AndroidManifest.xml` 檔案中，於 `<application>` 標籤下新增以下設定：
...
Flutter 在 Web 上提供了[兩種渲染器][two renderers] ——`canvaskit` 和 `skwasm`——目前兩者皆使用 Skia。未來有可能會改用 Impeller。
[two renderers]: /platform-integration/web/renderers#renderers

### macOS

你可以在 macOS 上透過旗標（flag）來嘗試啟用 Impeller。未來的版本中，將會移除選擇不使用 Impeller 的能力。
...
團隊持續在改進 Impeller 的支援。
如果你在任何平台上遇到 Impeller 的效能或畫面品質問題，
請在 [GitHub tracker][file-issue] 回報問題。
請在問題標題前加上 `[Impeller]`，
並附上一個簡單可重現的測試案例。

提交 Impeller 問題時，請提供以下資訊：

* 你執行的裝置，包括晶片資訊。
* 任何可見問題的截圖或錄影。
* [效能追蹤檔案的匯出][export of the performance trace]。
  請將檔案壓縮後附加在 GitHub 問題中。
...
* [如何設定 Xcode 以使用 metal 進行 GPU 畫面擷取][impeller-xcode-capture]
* [學習閱讀 GPU 畫面擷取][impeller-read-capture]
* [如何為命令列應用程式啟用 metal 驗證][impeller-metal-validation]
* [Impeller 如何因應 Open GL ES 2.0 缺乏 uniform buffers 的問題][impeller-ubo-gles2]
* [撰寫高效能著色器的建議][impeller-shader-optimization]
* [Impeller 的顏色混合運作方式][impeller-blending]
