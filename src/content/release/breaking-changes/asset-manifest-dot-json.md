...
這個檔案實際上包含了一份資源清單。
應用程式程式碼可以使用 [`AssetBundle`][`AssetBundle`] API 來讀取它，
以便在執行時判斷有哪些資源可用。
...
### 從 Flutter 應用程式外部的 Dart 程式碼讀取資源資訊清單
...
如果你需要從不是 Flutter 應用程式的程式碼中讀取這個檔案，因此無法使用 [`AssetManifest`][`AssetManifest`] API，你仍然可以自行解析這個檔案。

你可以使用 [`standard_message_codec`][`standard_message_codec`] 套件來解析其內容。
...
自 Flutter 3.19 之後的第四個穩定版發行起，或自 3.19 發行後滿一年（以較晚者為準），將不再產生 `AssetManifest.json`。
