...
以下方法已由 `flutter_test` 套件中的 API 取代：
...
作為底層外掛（plugin）通訊架構重構的一部分，我們已從先前的 `onPlatformMessage`/`handlePlatformMessage`
邏輯，轉移到由引擎中 `ChannelBuffers` 類別實作的每通道緩衝系統。
為了維持與現有程式碼的相容性，現有的 `BinaryMessenger.setMessageHandler` API
已重構為使用新的 `ChannelBuffers` API。

`ChannelBuffers` API 與先前 API 之間的一個差異在於，新 API 在處理非同步方面更加一致。其副作用是，
訊息傳遞相關的 API 現在完全採用非同步方式。

這對於舊有測試 API 的實作帶來了挑戰，這些 API 因歷史原因
原本位於 `flutter` 套件中。
由於它們依賴底層邏輯的部分同步特性，因此需要進行重構。
為了避免將更多測試邏輯加入 `flutter` 套件，
我們決定將這部分邏輯移至 `flutter_test` 套件。
...
這些替代方案僅適用於使用新 `TestDefaultBinaryMessengerBinding`
的程式碼（例如在 `flutter_test` 測試中使用 `testWidgets` 的任何程式碼）。
針對生產環境中使用這些 API 的程式碼，並無替代方案，因為這些 API 並非設計給生產環境使用。

使用 `checkMessageHandler` 的測試在新 API 中沒有對應功能，因為訊息處理器註冊
現在直接由 `ChannelBuffers` 物件處理，且不會
公開目前註冊於某個通道的監聽器。
（實際上，驗證處理器註冊的測試似乎相當罕見。）
...
此外，`onPlatformMessage` 回呼（callback）先前由框架掛載用於接收來自插件的訊息，現在已不再使用（未來也將被移除）。因此，呼叫此回呼以將訊息注入框架，將不再產生任何效果。

## 遷移指南

`flutter_test` 套件提供了一些 shim（兼容層），讓過時的 `setMock...` 和 `checkMock...` 方法仍可繼續運作。
先前未匯入 `package:flutter_test/flutter_test.dart` 的測試，
可以透過匯入該套件來啟用這些 shim；
這通常足以讓大多數程式碼完成遷移。

不過，這些 shim API 已經被標記為不建議使用（deprecated）。建議在使用 `WidgetTester` 的程式碼中（例如使用 `testWidgets`），改用以下模式來取代對這些方法的呼叫
（其中 `tester` 是 `WidgetTester` 實例）：
...
無法存取 `WidgetTester` 的程式碼，可以參考 `TestDefaultBinaryMessengerBinding.instance!.defaultBinaryMessenger`，而非 `tester.binding.defaultBinaryMessenger`。

未使用預設測試元件綁定（`AutomatedTestWidgetsFlutterBinding`，由 `testWidgets` 初始化）的測試，可以將 `TestDefaultBinaryMessengerBinding` mixin 混入其綁定中，以取得相同的結果。

操作 `onPlatformMessage` 的測試將無法如預期運作。若要向 framework 傳送模擬訊息，請考慮使用 `ChannelBuffers.push`。
在新的 API 中，沒有機制可攔截來自插件的訊息並將其轉發給 framework。
如果你的使用情境需要這樣的機制，請提交 bug 回報。
...
* [PR #76288: Migrate to ChannelBuffers.push][PR #76288: Migrate to ChannelBuffers.push]
