...
在 [`AppLifecycleState`][`AppLifecycleState`] 列舉（enum）中新增了一個 `hidden` 狀態，用於表示應用程式目前處於不可見狀態。

## 背景說明

`AppLifecycleState` 列舉（enum）用於指示當 [`WidgetsBindingObserver.didChangeAppLifecycleState`][`WidgetsBindingObserver.didChangeAppLifecycleState`] 被呼叫時，應用程式目前所處的生命週期狀態。
