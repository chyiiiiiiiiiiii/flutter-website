...
在 Flutter 中，[`Intent`][`Intent`] 是一個物件，通常會透過 [`Shortcuts`][`Shortcuts`] 元件（Widget）綁定至某個鍵盤組合鍵。
`Intent` 可以綁定至 [`Action`][`Action`]，
用於更新應用程式狀態或執行其他操作。
在使用此 API 的過程中，我們發現設計上有幾個缺點，因此我們對 Actions API 進行了更新，使其更易於使用與理解。

在先前的 Actions API 設計中，actions 是從 [`LocalKey`][`LocalKey`] 對應到 `ActionFactory`，每次呼叫 `invoke` 方法時都會建立一個新的 `Action`。
...
2. 從快捷鍵到 `Intent` 再到 `Action` 的對應關係並不總是很清楚，因為資料結構是將 LogicalKeySet => Intent，再由 `LocalKey` => `ActionFactory`。
   新的對應方式仍然是 `LogicalKeySet` 對應 `Intent`，但接著會將 `Type`（`Intent` 類型）對應到 `Action`，
   這樣的設計更直接且易讀，因為 intent 的類型會直接寫在對應表中。

3. 如果某個 action 的鍵盤綁定在元件階層的其他地方，`Intent` 不一定能取得決定 intent/action 是否啟用所需的狀態。
...
這些主要變更大多來自 [Revise Action API][Revise Action API] 及 [Make Action.enabled be isEnabled(Intent intent) instead][] 這兩個 PR，詳細內容可參考 [設計文件](/go/actions-and-shortcuts-design-revision)。

## 變更說明

以下是針對上述問題所做的調整：

1. 提供給 [`Actions`][`Actions`] 元件（Widget）的 `Map<LocalKey, ActionFactory>`，現在是一個 `Map<Type, Action<Intent>>`（其類型為要傳遞給 Action 的 Intent 類型）。
2. `isEnabled` 方法已從 `Intent` 類別移至 `Action` 類別。
3. `Action.invoke` 與 `Actions.invoke` 方法中的 `FocusNode` 參數已被移除。
4. 呼叫 action 時不再會建立新的 `Action` 實例。
5. `Intent` 建構函式中的 `LocalKey` 參數已被移除。
6. `CallbackAction` 的 `LocalKey` 參數已被移除。
7. `Action` 類別現在為泛型（`Action<T extends Intent>`），以提升型別安全性。
8. `CallbackAction` 所使用的 `OnInvokeCallback` 不再需要 `FocusNode` 參數。
9. `ActionDispatcher.invokeAction` 的簽章已變更，不再接受可選的 `FocusNode`，而是改為可選的 `BuildContext`。
10. `Action` 子類別中的 `LocalKey` 靜態常數（依慣例命名為 key）已被移除。
11. `Action.invoke` 與 `ActionDispatcher.invokeAction` 方法現在會以 `Object` 形式回傳 action 執行的結果。
12. `Action` 類別現在可以監聽狀態變化。
13. `ActionFactory` typedef 已被移除，因為已不再使用。
...
而產生的 action 可能會被 [`ActionDispatcher`][`ActionDispatcher`] 保留，以記錄狀態。
