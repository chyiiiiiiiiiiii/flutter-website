...
description: >-
  新增的 TextScaler 類別取代了 textScaleFactor 純量值，以因應 Android 14 非線性文字縮放的支援。
---
...
為了導入 [Android 14 非線性字型縮放][Android 14 nonlinear font scaling] 功能，Flutter 框架中所有出現的 `textScaleFactor` 均已被棄用，並由 `TextScaler` 取代。

## 背景

許多平台允許使用者在系統偏好設定中全域調整文字內容的縮放比例。過去，縮放策略是以一個名為 `textScaleFactor` 的單一 `double` 值來表示，因為文字縮放是成比例的：`scaledFontSize = textScaleFactor x unScaledFontSize`。例如，當 `textScaleFactor` 為 2.0，且開發者指定的字型大小為 14.0 時，實際字型大小為 2.0 x 14.0 = 28.0。

隨著 [Android 14 非線性字型縮放][Android 14 nonlinear font scaling] 的引入，較大的文字會以較小的比例縮放，以避免已經很大的文字被過度放大。傳統「成比例」縮放所使用的 `textScaleFactor` 純量值，已無法代表這種新的縮放策略。[以 `TextScaler` 取代 `textScaleFactor`][Replaces `textScaleFactor` with `TextScaler`] 的 pull request 新增了一個名為 `TextScaler` 的類別，以取代 `textScaleFactor`，為這項新功能做準備。非線性文字縮放則會在另一個 pull request 中引入。

## 變更說明

引入了一個新的介面 `TextScaler`，用於表示文字縮放策略。
...
如果你目前並未直接使用 `textScaleFactor`，而是將其傳遞給另一個接收 `textScaleFactor` 的 API，且該接收方 API 已經完成遷移，那麼遷移起來相對簡單：
...
如果你正在使用`textScaleFactor`來縮放非字體大小的尺寸，  
目前沒有通用的規則可以將程式碼遷移至非線性縮放，  
這可能需要你以不同的方式實作 UI。  
...
你可以選擇使用「有效」的文字縮放因子（text scale factor），方法是在字型大小 20 上套用`TextScaler`：`chatBoxSize * textScaler.scale(20) / 20`，或者
重新設計 UI，讓元件（Widget）自行決定其內在大小。

### 覆寫元件子樹中的文字縮放策略

若要覆寫元件子樹中現有的`TextScaler`，可以這樣覆寫`MediaQuery`：
...
`MediaQuery.withNoTextScaling`（會建立一個完全停用其子樹中文字縮放的元件），以及`MediaQuery.withClampedTextScaling`（會建立一個將縮放後字型大小限制在`[minScaleFactor * fontSize, maxScaleFactor * fontSize]`範圍內的元件），這兩者都是方便用法，涵蓋了常見需要覆寫文字縮放策略的情境。
...
如果你希望在你的應用程式完全遷移之前，暫時在 Android 14 上選擇不使用非線性文字縮放，可以在應用程式的元件（Widget）樹頂部放置一個經過修改的 `MediaQuery`：
...
這個技巧使用了已被棄用的 `textScaleFactor` API，一旦該 API 從 Flutter API 中移除後，此方法將無法再使用。

## 時程表

合併至版本：3.13.0-4.0.pre<br>  
穩定版釋出：3.16
...
* [`MediaQuery.textScalerOf`][`MediaQuery.textScalerOf`]
* [`MediaQuery.maybeTextScalerOf`][`MediaQuery.maybeTextScalerOf`]
* [`MediaQuery.withNoTextScaling`][`MediaQuery.withNoTextScaling`]
* [`MediaQuery.withClampedTextScaling`][`MediaQuery.withClampedTextScaling`] 
...
* [Replaces `textScaleFactor` with `TextScaler`][Replaces `textScaleFactor` with `TextScaler`]
