...
註解（Annotations）是在渲染階段指派給螢幕上特定區域的中繼資料。
...
從概念上來說，這改變了`AnnotatedRegionLayer.offset`和`size`的定義。它們過去代表「限制註解搜尋的裁剪矩形」，而現在則共同表示「註解物件的區域」。

## 遷移指南

如果你的程式碼有主動使用這個 local position，本質上很可能是直接與 layer 互動，因為若是透過 render objects 或元件（Widgets）取得，這個結果早已不可靠。為了維持先前的行為，你可以自行重新實作`AnnotatedRegionLayer`，讓其回傳 local position 時不再扣除 offset。
