...
description: >-
  以下在 Flutter 達到生命週期終止後，
  已從 Flutter 移除的已棄用 API。
---
...
`TextButton`、`ElevatedButton` 以及 `OutlinedButton` 這三個元件（Widgets）皆有一個
...
這是透過以 `backgroundColor`、`foregroundColor` 和 `disabledForegroundColor` 取代上述屬性來達成的。
...
`selectedRowColor` 屬性在 `ThemeData` 中於 v3.1 被標記為已淘汰。

此屬性已不再被框架使用，因為原本使用它的元件（Widgets）已遷移至其他元件主題，或是在新版 Material Design 規範下已不再需要此屬性。
...
* 在 [#109070][#109070] 中標記為已淘汰（Deprecated）
* 在 [#139080][#139080] 中移除
...
`NavigatorState` 的 `focusScopeNode` 屬性自 v3.1 起已被標記為已淘汰（Deprecated）。

此變更是為了解決由 `Navigator` 引入的 `FocusScopeNode` 所產生的多項問題。取而代之的是，`FocusScope` 現在會包覆最上層的 `Navigator`，並放在 `WidgetsApp` 之中。`NavigatorState` 也被調整為包含自己的 `FocusNode`，並可透過其 `FocusNode.enclosingScope` 來存取正確的 `FocusScopeNode`。
...
* 已在 [#109702][#109702] 標記為已淘汰（Deprecated）
* 已在 [#139260][#139260] 移除
...
`body` 屬性在 `PlatformMenuBar` 中於 v3.1 被標記為已淘汰（Deprecated）。

此變更是為了讓 `PlatformMenuBar` 與框架中其他元件（Widgets）一致，並將其重新命名為 `child`。
...
`WidgetInspectorService` 和 `WidgetInspectorServiceExtensions` 在 `setPubRootDirectories` 上的淘汰時程已延長一年，以便 IDE 及其他用戶進行遷移。
