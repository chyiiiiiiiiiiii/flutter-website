...
為了讓自訂 Activity 也能提供 Flutter 外掛所需的`Activity`生命週期事件（lifecycle events），並透過 [`ActivityAware`][`ActivityAware`] 介面， [`FlutterEngine`][`FlutterEngine`] 提供了一個 [`getActivityControlSurface()`][`getActivityControlSurface()`] API。
...
自 [Issue #21272][Issue #21272] 起，請不要再透過以下方式，將你的 Activity 附加到 [`FlutterEngine`][`FlutterEngine`]：
...
如果您有自己的 activity 持有 [`FlutterView`][`FlutterView`]，請將以下呼叫替換為：
...
在你透過呼叫 [`getActivityControlSurface()`][`getActivityControlSurface()`] 於 [`FlutterEngine`][`FlutterEngine`] 上所取得的 [`ActivityControlSurface`][`ActivityControlSurface`] 上。
...
以處理您的 activity 被另一個 activity 替換，並附加到 [`FlutterEngine`][`FlutterEngine`] 上的情境。
一般來說，您需要執行與 activity 被自然銷毀時相同的卸除（detaching）操作。
...
相關問題： [Issue #66192][Issue #66192] — 非專屬 UI 元件附加到 FlutterEngine 會導致事件串擾（event crosstalk）
