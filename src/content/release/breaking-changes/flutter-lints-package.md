...
[`package:flutter_lints`][`package:flutter_lints`] 定義了最新一套推薦的 lint 規則，
...
在 `package:flutter_lints` 推出之前，Flutter 框架內建了一套由 [`analysis_options_user.yaml`][`analysis_options_user.yaml`]
所定義的 lint 規則，這些規則會由 [dart analyzer][dart analyzer]
...
而不會破壞既有專案。由於該套件是建立在 Dart 的 [`package:lints`][`package:lints`] 之上，
