...
description: >
  現在大多數平台上的觸控板手勢會傳送`PointerPanZoom`序列，並可觸發平移、拖曳及縮放的`GestureRecognizer`回呼。
---
...
相關的`GestureRecognizer`也已更新，可註冊對觸控板手勢序列的關注，並在觸控板上有兩指或以上移動時，發出`onDrag`、`onPan` 和／或 `onScale`回呼。
...
`Listener` 現在新增了三個回呼：`onPointerPanZoomStart`、`onPointerPanZoomUpdate` 以及 `onPointerPanZoomEnd`，可用於觀察觸控板的滾動與縮放事件。
...
對 `addPointerPanZoom` 呼叫 `GestureRecognizer` 方法並傳入 `PointerPanZoomStartEvent`，將會讓辨識器註冊其對該觸控板互動的關注，
...
無需任何更動，`GestureDetector` 會自動處理觸控板手勢事件，並在辨識到時觸發回呼。
...
升級至 Flutter 3.3.0 之後，行為不會有變化，因為必須在每個 `GestureRecognizer` 上呼叫 `addPointerPanZoom`，才能讓其追蹤手勢。當使用觸控板捲動時，下列程式碼將不會收到平移（pan）手勢的回呼：
