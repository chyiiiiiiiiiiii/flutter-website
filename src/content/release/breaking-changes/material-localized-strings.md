...
[`ReorderableListView`][`ReorderableListView`] 使用這些字串來標註其語意動作。
若要將相同的標註應用於 [`ReorderableList`][`ReorderableList`]
和 [`SliverReorderableList`][`SliverReorderableList`]，則需要
從 Widgets 函式庫取得這些字串。
...
`reorderItemDown`、`reorderItemLeft` 以及 `reorderItemRight` 已被棄用，
並由 [`WidgetsLocalizations`][`WidgetsLocalizations`] 中的相同字串取代。

## 遷移指南

如果你在程式碼中有使用這些字串，
請改為從 `WidgetsLocalizations` 取得。
...
如果你有覆寫 `MaterialLocalizations` 或 `WidgetsLocalizations`，請務必將翻譯內容從 `MaterialLocalizations` 子類別中移除，並移至 `WidgetsLocalizations` 子類別。
