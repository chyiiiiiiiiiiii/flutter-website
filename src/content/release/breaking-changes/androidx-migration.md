...
[AndroidX][AndroidX] 是對原有 Android 支援函式庫（Android Support Library）的一項重大改進。
...
### 如何將現有的應用程式、套件（plugin）或可編輯主機模組（host-editable module）專案遷移至 AndroidX？
...
最後，如果您遷移的是套件（plugin），請將新的 AndroidX 版本發佈到 pub，並更新您的 `CHANGELOG.md`，以標示此新版本已支援 AndroidX。
...
將使用 Support Library 的 Flutter 套件
轉換為 AndroidX，因此即使這些套件
