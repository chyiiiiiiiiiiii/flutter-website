...
description: >-
  了解 Flutter 3.35 中 radio 元件 (Widget) 的變更內容。
---
...
引入了 `RadioGroup` 元件 (Widget)，用於集中管理 `groupValue`，以及一組 `Radio` 元件 (Widget) 的 `onChanged` 回呼 (callback)。因此，個別的 `Radio.groupValue` 和 `Radio.onChanged` 屬性已被棄用。

## 背景

為了符合 APG（ARIA Practices Guide）對於 radio 按鈕群組在鍵盤導覽和語意屬性的要求，Flutter 需要一個專用的 radio 群組概念。引入包裹元件 (Wrapper Widget) `RadioGroup`，即可直接支援這些功能。這項變更同時也讓個別 `Radio` 元件 (Widget) 的 API 得以簡化。
