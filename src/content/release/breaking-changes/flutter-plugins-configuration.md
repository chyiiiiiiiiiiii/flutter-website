...
description: >-
  已淘汰的 `.flutter-plugins` 工具檔案輸出已被 `.flutter-plugins-dependencies` 取代，任何建置腳本或相關引用也必須同步更新。
---
...
依賴 `.flutter-plugins` 存在的工具與建置腳本（如 Android 應用程式的 Gradle 設定）都需要進行更新。
...
同時產生這兩個檔案會造成技術債務，進而複雜化新功能的開發，例如在發佈版應用程式中不綑綁 `dev_dependency` 插件等需求。

## 遷移指南

大多數 Flutter 開發者不會解析或使用這個檔案，但建置設定可能會用到，包括由舊版 `flutter create --platforms android` 指令產生的 `settings.gradle` 檔案。
這些舊有檔案可能仍然參考 `.flutter-plugins`，因此必須更新為新版的建置腳本。
...
可能會升級為其 `settings.gradle.kts` 對應項目：
...
在此變更納入後的一個穩定版本，`.flutter-plugins`將不再產生。

## 參考資料

相關議題：

- [Issue 48918][Issue 48918]，於 2020 年時，`.flutter-plugins`被規劃為棄用。

相關 PR：

- [PR 45379][PR 45379]，最初加入了`.flutter-plugins-dependencies`。
- [PR 157388][PR 157388]，在 Flutter Android 建置腳本中新增了警告。
