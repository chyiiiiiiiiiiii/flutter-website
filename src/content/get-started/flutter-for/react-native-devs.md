...
和 React Native 類似，Flutter 採用反應式（reactive-style）視圖。不過，RN 會轉譯成原生元件（Widgets），而 Flutter 則是直接編譯為原生程式碼。Flutter 能夠控制螢幕上的每一個像素，因此不需要 JavaScript 橋接層，能夠避免由此產生的效能問題。
...
如需更多資訊，請參閱 [async and await][async and await] 的文件。
...
如需更多資訊，請參閱 [Getting started][Getting started]，該文件會引導你建立一個按鈕點擊計數器應用程式。建立一個 Flutter 專案會產生所有你需要的檔案，讓你可以在 Android 和 iOS 裝置上執行範例應用程式。
...
在 Flutter 中，若要使用 Material Design 元件 (Material components)，請匯入 `material.dart` 套件。若要使用 iOS 風格的元件，請匯入 Cupertino 函式庫。若想使用更基礎的元件組合，請匯入 Widgets 函式庫。  
...
在 Flutter 中，你可以使用核心元件 (Widgets) 函式庫中的 `Center` 和 `Text` 元件，建立一個相同的 "Hello world!" 應用程式。
...
現在你已經看過最基本的 Flutter 應用程式，下一節將說明如何善用 Flutter 豐富的元件（Widgets）函式庫，打造現代且精緻的應用程式。
...
以下圖片展示了由 Material Design 元件 (Widgets) 所構建的 "Hello world!" 範例。
...
`StatelessWidget` 顧名思義，就是沒有狀態的元件 (Widget)。
...
在簡單或基礎的應用程式中，巢狀元件 (Widgets) 很容易管理；
但隨著程式碼基礎擴大、應用程式變得複雜時，
你應該將巢狀過深的元件拆分為回傳元件的函式或更小的類別。
建立獨立的函式與元件 (Widgets) 可以讓你在應用程式內重複使用這些元件。
...
這些檔案也會與指定的資源一同包含進資源包中。Flutter 在為您的應用程式選擇適合解析度的圖片時，會使用資源變體（asset variants）。

在 React Native 中，您會將靜態圖片檔案放在原始碼目錄中，並直接引用它。
...
在 Flutter 中，您可以在元件（Widget）的 build 方法中，使用 `Image.asset` 建構函式，將靜態圖片加入您的應用程式。
...
如需更多資訊，請參閱 [Adding Assets and Images in Flutter][Adding Assets and Images in Flutter]。

### 如何透過網路載入圖片？

在 React Native 中，你會將 `uri` 指定在 `Image` 元件 (Component) 的 `source` 屬性中，並在需要時提供尺寸。

在 Flutter 中，請使用 `Image.network` 建構函式 (constructor) 來從 URL 載入圖片。
...
如需更多資訊，請參閱 [使用套件][Using Packages] 以及
[開發套件與外掛][Developing Packages & Plugins]。

你可以在 [pub.dev][pub.dev] 的 [Flutter 套件][Flutter packages] 區域找到許多由 Flutter 開發者分享的套件。

## Flutter 元件 (Widgets)

在 Flutter 中，你會使用元件（Widgets）來構建 UI，這些元件會根據當前的設定與狀態描述其視圖應該呈現的樣貌。
...
關於 `Widgets` 套件中的核心元件，請參閱 [Flutter 基本元件][Flutter Basic Widgets]、[Flutter 元件目錄][Flutter Widget Catalog] 或 [Flutter 元件索引][Flutter Widget Index]。
...
在 React Native 中，`FlatList` 或 `SectionList` 用於渲染簡單或分段的清單。
...
在 Flutter 中，你可以使用 [`CustomPaint`][`CustomPaint`] 和 [`CustomPainter`][`CustomPainter`] 類別來繪製 (Painting) 到畫布上。

以下範例展示了如何在繪製階段（paint phase）使用 `CustomPaint` 元件 (Widget) 進行繪製。這個範例實作了抽象類別 `CustomPainter`，並將其傳遞給 `CustomPaint` 的 painter 屬性。`CustomPaint` 子類別必須實作 `paint()` 和 `shouldRepaint()` 方法。
...
舉例來說，[`Column`][`Column`] 和 [`Row`][`Row`] 這兩個元件會接收一個 `children` 清單，分別將其垂直與水平排列。而 [`Container`][`Container`] 元件則可以結合多種版面配置與樣式屬性使用，[`Center`][`Center`] 元件則會將其子元件置中顯示。
...
Flutter 在其核心元件（Widgets）庫中提供了多種版面配置元件（Layout widgets）。
例如，[`Padding`][`Padding`]、[`Align`][`Align`] 和 [`Stack`][`Stack`]。

完整清單請參考 [Layout Widgets][Layout Widgets]。
...
Flutter 則使用 [`Stack`][`Stack`]
元件來將子元件（children widgets）以層疊方式排列。
...
在 Flutter 中，`Text` 元件（Widget）可以為其 style 屬性指定 `TextStyle` 類別。如果你想在多個地方使用相同的文字樣式，可以建立一個 [`TextStyle`][`TextStyle`] 類別，並將其套用於多個 `Text` 元件（Widget）。
...
在 Flutter 中，匯入 Materials 函式庫（Material library）時，也會一併引入豐富的 [Material icons（Material 圖示）][Material icons] 與 [colors（顏色）][colors]。
...
Flutter 的 [Cupertino (iOS-style)][Cupertino (iOS-style)] 套件提供高度還原的 iOS 當前設計語言元件（Widgets）。  
...
若要全域自訂元件（Widgets）的顏色與樣式，可以使用 `ThemeData` 來指定主題中各個層面的預設顏色。請將 `MaterialApp` 的 theme 屬性設為 `ThemeData` 物件。[`Colors`][`Colors`] 類別會提供來自 Material Design [色彩調色盤][color palette] 的顏色。
...
在 Flutter 中，幾乎所有東西都可以透過在 [`ThemeData`][`ThemeData`] 類別中定義樣式，並將其傳遞給 [`MaterialApp`][`MaterialApp`] 元件（Widget）的 theme 屬性，來建立一致的樣式。
...
[`Theme`][`Theme`] 元件會在其 `data` 參數中接收 `ThemeData`，
並將 `ThemeData` 套用到其所有子元件（children widgets）。
...
狀態（State）是指在元件（Widget）建立時可同步讀取的資訊，或是在元件生命週期內可能會改變的資訊。要在 Flutter 中管理應用程式狀態，請使用 [`StatefulWidget`][`StatefulWidget`] 搭配 State 物件。

如需更多關於在 Flutter 中管理狀態的方法，請參考 [State management][State management]。

### StatelessWidget（無狀態元件）

Flutter 中的 `StatelessWidget` 是一種不需要狀態變化的元件（Widget）——它本身沒有需要管理的內部狀態。

當你要描述的使用者介面部分僅依賴於物件本身的組態資訊，以及該元件被建立時的 [`BuildContext`][`BuildContext`]，而不需要依賴其他狀態時，無狀態元件（StatelessWidget）就非常實用。

[`AboutDialog`][`AboutDialog`]、[`CircleAvatar`][`CircleAvatar`] 和 [`Text`][`Text`] 都是繼承自 [`StatelessWidget`][`StatelessWidget`] 的無狀態元件（stateless widgets）範例。
...
* 當元件（Widget）被插入到樹中時
* 當元件的父元件變更其組態時
* 當其所依賴的 [`InheritedWidget`][`InheritedWidget`] 發生變化時

### StatefulWidget

[`StatefulWidget`][`StatefulWidget`] 是一個會改變狀態的元件（Widget）。
...
_State_（狀態）是指在元件（Widget）建立時可以同步讀取，且在元件生命週期內可能會改變的資訊。
元件實作者有責任確保當狀態變更時，狀態物件能夠及時收到通知。
當元件（Widget）會動態變化時，請使用 `StatefulWidget`。
例如，透過在表單中輸入文字或移動滑桿，元件的狀態會改變。
又或者，狀態也可能隨時間變化——例如資料來源更新 UI。

[`Checkbox`][`Checkbox`]、[`Radio`][`Radio`]、[`Slider`][`Slider`]、[`InkWell`][`InkWell`]、
[`Form`][`Form`] 和 [`TextField`][`TextField`]
都是繼承自 [`StatefulWidget`][`StatefulWidget`] 的 stateful 元件（stateful widgets）範例。
...
以下這個狀態類別 `_MyStatefulWidgetState`，為該元件（Widget）實作了 `build()` 方法。
...
在設計元件（Widget）時，請考慮以下幾點：

1. 判斷元件應該是 `StatefulWidget` 還是 `StatelessWidget`。

在 Flutter 中，元件（Widget）分為 Stateful 或 Stateless，這取決於它是否依賴狀態變化。
...
在 Flutter 中，請使用 [`shared_preferences`][`shared_preferences`] 套件來儲存與讀取應用程式全域且持久化的鍵值資料。`shared_preferences` 套件在 iOS 上包裝了 `NSUserDefaults`，在 Android 上則包裝了 `SharedPreferences`，為簡單資料提供持久化儲存功能。
...
在 iOS 中，每個新螢幕對應一個新的 ViewController。而在 Flutter 中，螢幕就是元件（Widgets）！要在 Flutter 中導覽到新的螢幕，可以使用 Navigator 元件（Navigator widget）。
...
在 Flutter 中，有兩個主要的元件（Widgets）用於在螢幕之間導覽：

* [`Route`][`Route`] 是對應用程式螢幕或頁面的抽象。
* [`Navigator`][`Navigator`] 是一個管理路由（Route）的元件（Widget）。

`Navigator` 被定義為一個以堆疊（stack）方式管理一組子元件（child widgets）的元件。Navigator 會管理一個 `Route` 物件的堆疊，並提供管理該堆疊的方法，例如 [`Navigator.push`][`Navigator.push`] 和 [`Navigator.pop`][`Navigator.pop`]。
路由（routes）清單可以在 [`MaterialApp`][`MaterialApp`] 元件中指定，或者也可以動態建立，例如在 hero 動畫（animation）中。
以下範例在 `MaterialApp` 元件中指定了命名路由（named routes）。

:::note
大多數應用程式現在已不再建議使用命名路由（named routes）。
如需更多資訊，請參閱 [Limitations][Limitations]（限制）於 [navigation overview][navigation overview]（導覽總覽）頁面。
:::
...
要導向至命名路由（named route），會使用 [`Navigator.of()`][`Navigator.of()`] 方法來指定 `BuildContext`（在元件樹中某個元件位置的控制代碼）。將路由名稱傳遞給 `pushNamed` 函式，即可導向至指定的路由。
...
它會將指定的 [`Route`][`Route`] 新增到最緊密包覆指定 [`BuildContext`][`BuildContext`] 的 Navigator 的歷史紀錄中，
並切換到該路由。在下列範例中，
...
[`TickerProvider`][`TickerProvider`] 是一個由能夠提供 [`Ticker`][`Ticker`] 物件的類別所實作的介面。
Ticker 可被任何需要在每次畫面（frame）觸發時收到通知的物件使用，但最常見的是透過 [`AnimationController`][`AnimationController`] 間接使用。`AnimationController` 需要 `TickerProvider` 來取得它們的 `Ticker`。
...
[`Scaffold`][`Scaffold`] 元件（Widget）會包裹一個新的 `TabBar` 元件，並建立兩個分頁（tabs）。
`TabBarView` 元件會作為 `Scaffold` 元件的 `body` 參數傳入。
所有對應於 `TabBar` 元件分頁的螢幕（screens）都會作為 `TabBarView` 元件的子元件（children），並且共用相同的 `TabController`。
...
在 Flutter 中，我們可以將 `Drawer` 元件（Widget）與 `Scaffold` 結合使用，以建立具有 Material Design 抽屜（drawer）的版面配置。  
...
`Drawer` 元件是一個 Material Design 面板，會從 `Scaffold` 邊緣以水平方式滑入，用來在應用程式中顯示導覽連結。你可以提供一個 [`ElevatedButton`][`ElevatedButton`]、[`Text`][`Text`] 元件，或是一個項目清單作為 `Drawer` 元件的子項（child）。  
在下列範例中，[`ListTile`][`ListTile`] 元件提供了點擊時的導覽功能。
...
`Scaffold` 元件（Widget）同時包含一個 `AppBar` 元件，當 `Scaffold` 中有 Drawer 可用時，會自動顯示一個合適的 IconButton 來顯示 `Drawer`。`Scaffold` 也會自動處理邊緣滑動手勢，以顯示 `Drawer`。
...
對於更複雜的手勢，以及將多個觸控組合成單一手勢時，會使用 [`PanResponder`][`PanResponder`]。
...
文字欄位（text field）讓使用者可以在你的應用程式中輸入文字，因此可以用來建立表單、即時通訊應用、搜尋體驗等。Flutter 提供兩個核心的文字欄位元件（text field widgets）：[`TextField`][`TextField`] 和 [`TextFormField`][`TextFormField`]。
...
在這個範例中，當使用者點擊提交按鈕時，會跳出一個警示對話框（alert dialog），顯示目前在文字欄位 (text field) 中輸入的文字。
這是透過 [`AlertDialog`][`AlertDialog`] 元件（Widget）來顯示警示訊息實現的，而`TextField`中的文字則是透過 [`TextEditingController`][`TextEditingController`] 的 `text` 屬性來取得。

### 如何使用 Form 元件（Widgets）？

在 Flutter 中，可以使用 [`Form`][`Form`] 元件（Widget），並將 [`TextFormField`][`TextFormField`] 元件（Widgets）和提交按鈕作為 `children` 傳入。
`TextFormField` 元件有一個名為 [`onSaved`][`onSaved`] 的參數，該參數接受一個 callback，並在表單被儲存時執行。`FormState` 物件可用於儲存、重設或驗證每一個作為此 `Form` 子孫的 `FormField`。
...
在開發跨平台應用程式時，您會希望盡可能在多個平台間重複使用相同的程式碼。不過，有些情境下，根據作業系統（OS）採用不同的實作方式會更合適。這時就需要針對特定平台宣告，並分別實作。
...
類別以及 [`AnimationController`][`AnimationController`] 類別。
...
要在 Flutter 中建立相同的動畫，請建立一個名為 `controller` 的 [`AnimationController`][`AnimationController`] 物件，並指定持續時間。預設情況下，`AnimationController` 會在指定的持續時間內，線性產生從 0.0 到 1.0 的數值。每當執行您應用程式的裝置準備好顯示新畫面時，動畫控制器就會產生一個新數值。通常這個速率約為每秒 60 個數值。

在定義 `AnimationController` 時，您必須傳入一個 `vsync` 物件。`vsync` 的存在可以防止螢幕外的動畫消耗不必要的資源。您可以透過在類別定義中加入 `TickerProviderStateMixin`，將您的 stateful 物件作為 `vsync` 使用。`AnimationController` 需要一個 TickerProvider，這可以透過建構函式的 `vsync` 參數來設定。

[`Tween`][`Tween`] 描述了起始值與結束值之間的插值，或是從輸入範圍對應到輸出範圍的映射。若要將 `Tween` 物件用於動畫，請呼叫 `Tween` 物件的 `animate()` 方法，並傳入您想要修改的 `Animation` 物件。

在此範例中，使用 [`FadeTransition`][`FadeTransition`] 元件，並將 `opacity` 屬性對應到 `animation` 物件。

若要啟動動畫，請使用 `controller.forward()`。您也可以透過控制器執行其他操作，例如 `fling()` 或 `repeat()`。

在本範例中，[`FlutterLogo`][`FlutterLogo`] 元件會被用在 `FadeTransition` 元件內。
...
下表列出了常用的 React Native 元件，對應到 Flutter 中的元件（Widgets）及常見的元件屬性。
...
|                                                                                      | Child                                                                              | 按鈕的標籤。                                                                                                      |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`ScrollView`](https://reactnative.dev/docs/scrollview)                | [`ListView`][`ListView`]                                    | 以線性方式排列的可滾動元件（Widget）清單。|
||        children                                                                              | 	( <Widget\> [ ])  要顯示的子元件（Widget）清單。
||controller |[ [`ScrollController`][ [`ScrollController`] ] 可用於控制可滾動元件的物件。
||itemExtent|[ double ] 若非 null，強制所有子元件在滾動方向上具有指定長度。
||scroll Direction|[ [`Axis`][ [`Axis`] ] 滾動檢視的滾動軸向。
||                                                                                                            |                                                                                                                                        |
| [`FlatList`](https://reactnative.dev/docs/flatlist)                    | [`ListView.builder`][`ListView.builder`]               | 按需建立線性元件陣列的建構函式。
...
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`Image`](https://reactnative.dev/docs/image)                         | [`Image`][`Image`]                                           | 用於顯示圖片的元件。                                                                                                       |
|                                                                                      |  image [required]                                                                                          | 要顯示的圖片。                                                                                                                  |
|                                                                                      | Image. asset                                                                                                | 提供多種建構函式以支援不同圖片來源。                                                 |
|                                                                                      | width, height, color, alignment                                                                            | 圖片的樣式與版面配置。                                                                                                         |
|                                                                                      | fit                                                                                                        | 將圖片內嵌於版面配置分配的空間內。                                                                           |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`Modal`](https://reactnative.dev/docs/modal)                          | [`ModalRoute`][`ModalRoute`]                                | 阻擋與前一個路由互動的 Route。                                                                                  |
|                                                                                      | animation                                                                                                  | 控制本路由轉場及前一個路由前進轉場的動畫（animation）。                                          |
|                                                                                      |                                                                                                            |                                                                                                                                        |
|  [`ActivityIndicator`](https://reactnative.dev/docs/activityindicator) | [`CircularProgressIndicator`][`CircularProgressIndicator`] | 顯示圓形進度的元件。                                                                                           |
|                                                                                      | strokeWidth                                                                                                | 畫圓時線條的寬度。                                                                                         |
|                                                                                      | backgroundColor                                                                                            | 進度指示器的背景顏色，預設為目前主題的 `ThemeData.backgroundColor`。                                   |
|                                                                                      |                                                                                                            |                                                                                                                                        |
|  [`ActivityIndicator`](https://reactnative.dev/docs/activityindicator) | [`LinearProgressIndicator`][`LinearProgressIndicator`]     | 顯示線性進度的元件。                                                                                           |
|                                                                                      | value                                                                                                      | 此進度指示器的值。                                                                                                   |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`RefreshControl`](https://reactnative.dev/docs/refreshcontrol)        | [`RefreshIndicator`][`RefreshIndicator`]                   | 支援 Material 設計「滑動以重新整理」操作的元件。                                                                          |
|                                                                                      | color                                                                                                      | 進度指示器的前景顏色。                                                                                             |
|                                                                                      | onRefresh                                                                                                  | 當使用者拖曳刷新指示器到足夠距離以觸發應用程式重新整理時呼叫的函式。  |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`View`](https://reactnative.dev/docs/view)                            | [`Container`][`Container`]                                  | 包裹子元件（Widget）的元件。                                                                                                                |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`View`](https://reactnative.dev/docs/view)                            | [`Column`][`Column`]                                        | 垂直排列子元件的元件。                                                                                              |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`View`](https://reactnative.dev/docs/view)                            | [`Row`][`Row`]                                              | 水平排列子元件的元件。                                                                                            |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`View`](https://reactnative.dev/docs/view)                            | [`Center`][`Center`]                                        | 將子元件置中的元件。                                                                                                       |
|                                                                                      |                                                                                                            |                                                                                                                                        |
| [`View`](https://reactnative.dev/docs/view)                            | [`Padding`][`Padding`]                                      | 以指定 padding 內距包裹子元件的元件。                                                                                                 |
|                                                                                      | padding [required]                                                                                         | [ EdgeInsets ] 內縮子元件的空間大小。
|||
| [`TouchableOpacity`](https://reactnative.dev/docs/touchableopacity)    | [`GestureDetector`][`GestureDetector`]                      | 偵測手勢的元件。                                                                                                                       |
|                                                                                      | onTap                                                                                                      | 點擊時的回呼函式。                                                                                                               |
|                                                                                      | onDoubleTap                                                                                                | 在同一位置快速連續點擊兩次時的回呼函式。
|||
| [`TextInput`](https://reactnative.dev/docs/textinput)                | [`TextInput`][`TextInput`]                                   | 系統文字輸入控制介面的元件。                                                                                           |
|                                                                                      | controller                                                                                                 | [ [`TextEditingController`][ [`TextEditingController`] ] 用於存取與修改文字。
|||
| [`Text`](https://reactnative.dev/docs/text)                          | [`Text`][`Text`]                                            | 顯示單一樣式文字的 Text 元件。                                                                                                                                                                           |
|                                                                                      | data                                                                                                      | [ String ] 要顯示的文字。                                                                                                                                                                              |
|                                                                                      | textDirection                                                                                             | [ [`TextAlign`][ [`TextAlign`] ] 文字的流向。                                                                                     |
|                                                                                      |                                                                                                           |                                                                                                                                                                                                              |
| [`Switch`](https://reactnative.dev/docs/switch)                      | [`Switch`][`Switch`]                                      | Material Design 樣式的開關元件。                                                                                                                                                                                    |
