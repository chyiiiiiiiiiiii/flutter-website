...
Flutter 支援 Web 無障礙，方法是將其內部的 Semantics 樹轉譯為可供螢幕閱讀器理解的 HTML DOM 結構。
...
若要開啟無障礙功能，使用者需要使用 `aria-label="Enable accessibility"` 按下隱形按鈕。
按下此按鈕後，DOM 樹將會反映所有元件 (Widgets) 的無障礙資訊。
...
雖然 Flutter 的標準元件（Widgets）通常會自動提供這些語意資訊，但如果自訂元件沒有明確定義角色，對於螢幕閱讀器使用者來說，可能會變得難以理解。
...
Flutter 提供了 [`Semantics` 元件（Widget）][`Semantics` widget] 搭配 [`SemanticsRole` 列舉（enum）][`SemanticsRole` enum]，讓開發者能為自己的元件（Widgets）指派特定角色。當你的 Flutter web 應用程式被渲染時，這些 Flutter 專屬的角色會被轉換為對應的 ARIA 角色，並反映在網頁的 HTML 結構中。
