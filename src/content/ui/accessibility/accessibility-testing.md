...
以下程式碼片段展示如何在名為 `AccessibleApp` 的範例元件上使用 Guideline API：
...
該應用程式主螢幕上的每個按鈕都作為可點擊的目標，並以 18 點字型渲染文字。

你可以將 Guideline API 測試與其他 [元件測試][widget tests] 一起新增，或是在獨立的檔案中，例如本範例中的 `test/a11y_test.dart`。
...
你可以透過視覺化網頁應用程式所建立的語意節點來進行無障礙功能除錯，
