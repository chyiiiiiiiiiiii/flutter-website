...
在行動裝置上，螢幕閱讀器（如 [TalkBack][TalkBack]、[VoiceOver][VoiceOver]）能讓視障使用者透過語音回饋瞭解螢幕內容，並可藉由手勢（行動裝置）或鍵盤快速鍵（桌面裝置）與 UI 互動。請在您的行動裝置上開啟 VoiceOver 或 TalkBack，並在您的應用程式中進行瀏覽。
...
{% ytEmbed 'ROIe49kXOc8', '如何使用 VoiceOver 瀏覽您的 iPhone 或 iPad' %}
...
Windows 內建有一個螢幕閱讀器稱為 Narrator，但有些開發者推薦使用更受歡迎的 NVDA 螢幕閱讀器。若想了解如何使用 NVDA 測試 Windows 應用程式，請參考 [Screen Readers 101 For Front-End Developers (Windows)][nvda]。
...
Flutter 的標準元件 (Widgets) 會自動產生無障礙樹。不過，如果你的應用程式有不同需求，可以透過 [`Semantics` 元件][`Semantics` widget] 進行自訂。

當你的應用程式中有需要以特定語音朗讀的文字時，請呼叫 [`TextSpan.locale`][`TextSpan.locale`]，告知螢幕閱讀器應使用哪一種語音。
