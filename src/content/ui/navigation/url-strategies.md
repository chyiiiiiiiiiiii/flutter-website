...
[flutter_web_plugins][flutter_web_plugins] 函式庫（屬於 Flutter SDK）所提供的
[usePathUrlStrategy][usePathUrlStrategy] 函式。

你不能直接使用 `pub add` 新增 `flutter_web_plugins`。
請在你的 `pubspec.yaml` 檔案中，將其作為 Flutter [SDK 相依套件][SDK dependency] 引入：
...
PathUrlStrategy 使用 [History API][History API]，這需要對網頁伺服器進行額外的設定。

若要設定您的網頁伺服器以支援 PathUrlStrategy，請參考您的網頁伺服器文件，將請求重新導向到 `index.html`。請查閱您的網頁伺服器文件，瞭解如何設定單頁應用程式（Single-page apps）。

如果您使用 Firebase Hosting，初始化專案時請選擇「Configure as a single-page app」選項。更多資訊請參考 Firebase 的 [Configure rewrites][Configure rewrites] 文件。

透過執行 `flutter run -d chrome` 建立的本地開發伺服器已設定為能夠妥善處理任何路徑，並回退到您的應用程式的 `index.html` 檔案。

## 在非根目錄位置部署 Flutter 應用程式

請將 `<base href="/">` 標籤於 `web/index.html`
更新為您的應用程式所部署的路徑。
例如，若要將 Flutter 應用程式部署於
`my_app.dev/flutter_app`，請將
此標籤改為 `<base href="/flutter_app/">`。

針對正式發行版本，支援相對 `base href` 標籤，但必須考慮到頁面實際服務的完整 URL。
這代表對於 `/flutter_app/`、
`/flutter_app/nested/route` 以及 `/flutter_app/nested/route/` 的請求，所需的相對 `base href` 會有所不同
（例如分別為 `"."`、`".."` 及 `"../.."`）。
