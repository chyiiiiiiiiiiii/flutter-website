...
要在 VS Code 中使用 DevTools，你需要安裝 [Dart 擴充功能][Dart extension]。
如果你正在除錯 Flutter 應用程式，也建議安裝
[Flutter 擴充功能][Flutter extension]。

## 啟動應用程式進行除錯 {: #run-and-debug}

在 VS Code 中開啟專案的根目錄（包含 `pubspec.yaml` 的那個資料夾），
然後點選 **執行 > 開始除錯**（`F5`），即可啟動應用程式的除錯階段。

## 啟動 DevTools

當除錯階段啟動且應用程式已開始執行後，
**Open DevTools** 指令就會出現在
VS Code 指令選單（`F1`）中：
...
並且利用 `dart.devToolsLocation` 設定，控制 DevTools 是以完整視窗開啟，
還是作為新欄位顯示在目前編輯器旁。

完整的 Dart/Flutter 設定清單可參考
...
部分 Dart/Flutter 在 VS Code 的推薦設定也可在
[dartcode.org](https://dartcode.org/docs/recommended-settings/) 找到。

你也可以在語言狀態區（狀態列上 **Dart** 旁的 `{}` 圖示）
查看 DevTools 是否正在執行，並從這裡於瀏覽器中啟動它。
