...
**解答**：效能問題可能導致 [UI 畫格][UI frames] 卡頓，或讓某些操作變慢。
  ...
  2. 若想了解哪些程式碼在背景中耗費最多 CPU 時間，請使用 [CPU profiler][CPU profiler]。

更多資訊請參考 [Performance][Performance] 頁面。
...
如需更多有關 Flutter 應用程式除錯與效能分析的資訊，請參閱 [Debugging][Debugging] 頁面，特別是其中的 [其他資源][other resources] 列表。
