...
- 使用 [DevTools][DevTools]，這是一套在瀏覽器中執行的除錯與效能分析工具。
- 使用 VS Code 內建的除錯功能，
  例如設定中斷點。

以下說明的是 VS Code 中可用的功能。
如需啟動與使用 DevTools 的相關資訊，請參閱
[從 VS Code 執行 DevTools][Running DevTools from VS Code] 於 [DevTools][DevTools] 文件中。
:::
...
在除錯階段，[Command Palette][Command Palette] 及 [Flutter inspector][Flutter inspector] 會新增多項除錯指令。
當空間有限時，會以圖示方式顯示標籤。
...
進行這些變更時，請重新啟動你的應用程式，但不需結束除錯階段。要執行熱重啟（hot restart），請在 [Command Palette][Command Palette] 執行 **Flutter: Hot Restart** 指令。
