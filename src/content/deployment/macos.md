...
: 您的應用程式支援的最低 macOS 版本。
  若要查詢 Flutter 支援部署到哪些 macOS 版本，請參閱 Flutter 的 [Supported deployment platforms][Supported deployment platforms]。
...
如需更多關於 iOS 版本管理的資訊，請參閱 Apple Developer 網站上的 [Core Foundation Keys][Core Foundation Keys]。
...
[TestFlight][TestFlight] 讓開發者可以將應用程式推送給內部和外部測試人員。這個可選步驟說明如何在 TestFlight 上發佈您的建置版本。
