...
本指南將逐步說明如何將 Flutter 應用程式發布到 [App Store][appstore] 和 [TestFlight][TestFlight]。
...
透過 [App Store Connect][appstoreconnect] (formerly iTunes Connect) 管理您的應用程式生命週期。
...
: 您的應用程式支援的最低 iOS 版本。Flutter 支援 iOS 13 及以上版本。如果您的應用程式或套件包含使用 iOS 12 之後 API 的 Objective-C 或 Swift 程式碼，請將此設定更新為所需的最高版本。
...
你可以在 Apple Developer 網站上的 [Core Foundation Keys][Core Foundation Keys]
閱讀更多關於 iOS 版本管理的資訊。
...
之後，你可以讓命令列介面 (Command Line Interface, CLI) 工具根據私鑰自動建立新的 iOS 發佈憑證（iOS Distribution）。
...
[TestFlight][TestFlight] 讓開發者可以將應用程式推送給內部與外部測試者。這個選擇性步驟說明如何在 TestFlight 上釋出你的建置版本。
