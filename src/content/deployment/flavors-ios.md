...
1.  如果你正在處理一個已存在且至少有一個 Podfile 的 Flutter 專案，請更新該 Podfile。更多資訊請參閱 [Update Podfiles][Update Podfiles]。
...
2.  在工具列中，於 scheme 名稱旁邊，選擇你要測試的裝置。
...
3.  針對每個 scheme（`staging`、`production`）啟動 App，並確認每個 App 的顯示名稱是否已正確變更。如何啟動 scheme，請參考[啟動 Xcode scheme 的步驟][Launch an Xcode scheme]。
...
1.  準備你的圖示：
    ...
    * 產生 staging 與 production 圖示所需尺寸的版本，並以 PNG 格式儲存。
      ...
      :::note
      你可以使用像是 [App Icon Generator][App Icon Generator] 這類工具來產生各尺寸的圖示。
      :::
...
4.  針對每個 scheme（`staging`、`production`）啟動 App，並確認每個 App 的圖示是否已正確變更。如何啟動 scheme，請參考[啟動 Xcode scheme 的步驟][Launch an Xcode scheme]。
...
5.  展開 **Product Bundle Identifier** 設定，以檢視不同的建置組態。

6.  針對每個 scheme 的建置組態，設定所需的 bundle identifier。例如：
...
7.  確認這些 bundle identifier 已包含在你的 App ID 中，並且你的 App ID 已[註冊到 Apple Developer 帳戶][registered in your Apple Developer account]。

[registered in your Apple Developer account]: https://developer.apple.com/help/account/identifiers/register-an-app-id/

### 資源（Assets）打包

如果你的應用程式中有僅特定 flavor 會用到的資源，可以設定僅在啟動該 flavor 時才將這些資源打包進 App，避免未使用的資源讓 App bundle 檔案變大。若要為每個 flavor 打包資源，請在專案的 pubspec 中，於 `assets` 欄位下新增 `flavors` 子欄位。欲了解更多資訊，請參閱 [`assets` 欄位][`assets` field] 以及 [Flutter pubspec options][Flutter pubspec options]。
...
你可以使用 [build settings][build settings] 來管理你的 iOS 建置流程，從編譯、連結到除錯與發佈。

你可以將 build settings（建置設定）與 Flutter flavors 搭配使用的一種方式，是將這些 build settings 指派給 Xcode 的 build configurations（建置組態）。

例如，你可能會想要為 `Debug-staging` 和 `Debug-production` 指派不同的 API URL。例如： 
...
* [如何使用 FlutterFire CLI 設定 Flutter & Firebase 多重 flavor][flutterfireCLI]
