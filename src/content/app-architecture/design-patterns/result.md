...
如同在 [錯誤處理文件][Error handling documentation] 中所提到，
...
`Result`是一個[`sealed`][`sealed`]類別，可以繼承`Ok`或`Error`類別。若要回傳成功的值，請使用`Ok`子類別；若要回傳捕獲到的錯誤，則使用`Error`子類別。
...
這讓程式碼可以使用  
[switch result or expression][switch result or expression] 來判斷結果。
...
下方可以看到完整的 `Result` 類別，  
這是在 [Compass App example][Compass App example]  
針對 [Flutter architecture guidelines][Flutter architecture guidelines] 所實作的版本。

:::note
你也可以在 [pub.dev][pub.dev] 上找到不同現成可用的  
`Result` 類別實作，  
例如 [`result_dart`][`result_dart`]、[`result_type`][`result_type`] 與 [`multiple_result`][`multiple_result`] 套件。
:::
