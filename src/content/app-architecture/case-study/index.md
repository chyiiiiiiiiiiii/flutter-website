...
* 如何利用 [`ChangeNotifier`][`ChangeNotifier`] 與 [`Listenable`][`Listenable`] 物件來管理狀態
...
* UI 資料夾還包含一個名為 "core" 的子目錄。
  core 內含多個 view 共用的元件 (Widgets) 和主題邏輯，
  例如具有品牌樣式的按鈕。
...
本案例研究中的範例展示了一個應用程式如何遵循我們建議的架構規則，但其實還有許多其他範例應用程式可以實作。本應用程式的 UI 主要依賴 view model 和 `ChangeNotifier`，但同樣也可以用 streams，或其他像 [`riverpod`][`riverpod`]、[`flutter_bloc`][`flutter_bloc`] 和 [`signals`][`signals`] 套件所提供的函式庫來實作。
